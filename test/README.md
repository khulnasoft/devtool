...
This directory contains Devtool's integration tests, including the framework that makes them possible.

Integration tests work by instrumenting Devtool's components to modify and verify its state.
...
| start workspace | obtaining a Devtool API token, calling "createWorkspace" and watching for successful startup events.                                       |
...
- DB access to the Devtool DB

# Running the tests

## Automatically at Devtool
...
You may want to run tests to assert whether a Devtool installation is successfully integrated.
...
This is best for when you're actively developing Devtool.

Test will work if images that they use are already cached by Devtool instance. If not, they might fail if it takes too long to pull an image.
...
2. Tests that require correct user (user should have github OAuth integration setup with devtool). Run those with `-username=<devtool_username>`. Make sure to load https://github.com/khulnasoft/devtool-test-repo and https://github.com/khulnasoft/devtool workspaces inside your devtool that you are testing to preload those images onto your node. Wait for it to finish pulling those image, this will ensure that test will not fail due to timeout while waiting to pull an image for the first time.
...
2. In your Devtool (preview) environment, log into the preview environment, set `USER_NAME` to the user you logged in with, and set `USER_TOKEN` to any (does not have to be valid).
...
go test -v ./... \
    -run <test> \
    -namespace=default \
    -username=<devtool_user_with_oauth_setup> \
