...
func Setup(ctx context.Context) (string, string, env.Environment, bool, string, bool) {
	var (
		...
		enterprise      bool
		gitlab          bool
		waitDevtoolReady time.Duration
	...
	if defaultKubeConfig == "" {
		defaultKubeConfig = "/home/devtool/.kube/config"
	}
	...
	flagset.DurationVar(&waitDevtoolReady, "wait-devtool-timeout", 5*time.Minute, `wait time for Devtool components before starting integration test`)
	...
	testenv.Setup(
		waitOnDevtoolRunning(e.Namespace(), waitDevtoolReady),
	)
...
func waitOnDevtoolRunning(namespace string, waitTimeout time.Duration) env.Func {
	klog.V(2).Info("Checking status of Devtool components...")
	return func(ctx context.Context, cfg *envconf.Config) (context.Context, error) {
		...
		err := wait.PollImmediate(1*time.Second, waitTimeout, func() (bool, error) {
			...
			klog.V(2).Info("All Devtool components are running...")
			return true, nil
		})
		if err != nil {
			return ctx, nil
...
func logDevtoolStatus(t *testing.T, client klient.Client, namespace string) {
	var allPods corev1.PodList
	err := client.Resources(namespace).List(context.Background(), &allPods)
	if err != nil {
		t.Logf("failed to list pods to log devtool status: %v", err)
		return
	}
	...
	tw.Flush()
	t.Logf("Devtool components status:\n" + buf.String())
}
