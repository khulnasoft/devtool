...
func LaunchWorkspaceDirectly(t *testing.T, ctx context.Context, api *ComponentAPI, opts ...LaunchWorkspaceDirectlyOpt) (*LaunchWorkspaceDirectlyResult, StopWorkspaceFunc, error) {
	...
	options := launchWorkspaceDirectlyOptions{
		BaseImage: "docker.io/devtool/workspace-full:latest",
	}
	for _, o := range opts {
		err := o(&options)
		if err != nil {
	...
	req := &wsmanapi.StartWorkspaceRequest{
		...
		Spec: &wsmanapi.StartWorkspaceSpec{
			...
			Git: &wsmanapi.GitSpec{
				...
				Email:    "integration-test@devtool.io",
			},
			...
			Envvars: []*wsmanapi.EnvironmentVariable{
				...
				// from ws-manager in these tests we need to set it here ourselves.
				{
					...
					Value: "https://open-vsx.devtool.io/",
				},
			},
		},
	}
...
// LaunchWorkspaceFromContextURL force-creates a new workspace using the Devtool server API,
...
func LaunchWorkspaceFromContextURL(t *testing.T, ctx context.Context, contextURL string, username string, api *ComponentAPI, serverOpts ...DevtoolServerOpt) (*protocol.WorkspaceInfo, StopWorkspaceFunc, error) {
...
// LaunchWorkspaceWithOptions force-creates a new workspace using the Devtool server API,
// and waits for the workspace to start. If any step along the way fails, this function will
// fail the test.
//
// When possible, prefer the less complex LaunchWorkspaceDirectly.
func LaunchWorkspaceWithOptions(t *testing.T, ctx context.Context, opts *LaunchWorkspaceOptions, username string, api *ComponentAPI, serverOpts ...DevtoolServerOpt) (*protocol.WorkspaceInfo, StopWorkspaceFunc, error) {
	var (
		defaultServerOpts []DevtoolServerOpt
		stopWs            StopWorkspaceFunc = nil
		err               error
	)

	if username != "" {
		defaultServerOpts = []DevtoolServerOpt{WithDevtoolUser(username)}
	}
	...
	server, err := api.DevtoolServer(append(defaultServerOpts, serverOpts...)...)
	if err != nil {
	...
	for i := 0; i < 3; i++ {
		...
		if err != nil {
			...
			if scode == codes.NotFound || scode == codes.Unavailable {
				t.Log("retry strarting a workspace because cannnot start workspace: %w", err)
				time.Sleep(1 * time.Second)
				api.ClearDevtoolServerClientCache()
				server, err = api.DevtoolServer(append(defaultServerOpts, serverOpts...)...)
				if err != nil {
					return nil, nil, xerrors.Errorf("cannot start server: %w", err)
				}
