...
func TestRegularWorkspacePorts(t *testing.T) {
	...
	// This branch exposes a python server on port 3000 as part of the Devtool tasks.
	testRepo := "https://github.com/khulnasoft/devtool-test-repo/tree/integration-test/ports"
	testRepoName := "devtool-test-repo"
	...
	f := features.New("ports").
		...
		Assess("it can open and access workspace ports", func(testCtx context.Context, t *testing.T, cfg *envconf.Config) context.Context {
			...
			serverOpts := []integration.DevtoolServerOpt{integration.WithDevtoolUser(username)}
			server, err := api.DevtoolServer(serverOpts...)
			if err != nil {
				t.Fatal(err)
			}

			// Must change supervisor address from localhost to 10.0.5.2.
			err = server.SetEnvVar(ctx, &devtool.UserEnvVarValue{
			...
			defer func() {
				err := server.DeleteEnvVar(ctx, &devtool.UserEnvVarValue{
					Name:              "SUPERVISOR_ADDR",
					RepositoryPattern: "khulnasoft/" + testRepoName,
				})
				if err != nil {
			...
			nfo, stopWs, err := integration.LaunchWorkspaceFromContextURL(t, ctx, testRepo, username, api, integration.WithDevtoolUser(username))
			if err != nil {
				t.Fatal(err)
			}
