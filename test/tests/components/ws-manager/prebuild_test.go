...
const (
	prebuildLogPath string = "/workspace/.devtool"
		...
		Assess("it should open workspace from prebuild successfully", func(testCtx context.Context, t *testing.T, cfg *envconf.Config) context.Context {
			...
			for _, test := range tests {
				...
				t.Run(test.Name, func(t *testing.T) {
					...
					// check the folder permission is devtool
					checkFolderPermission(t, rsa, "/workspace")

					// check the files/folders permission under .git/ is devtool
...
// - make sure the workspace's init task ran (the init task will create a file `incremental.txt` see https://github.com/khulnasoft/test-incremental-workspace/blob/main/.devtool.yml)
...
func checkPrebuildLogExist(t *testing.T, cfg *envconf.Config, rsa *integration.RpcClient, ws *integration.LaunchWorkspaceDirectlyResult, wsRoot string) {
	...
	// we fall back to check the init task message within the /workspace/.devtool/prebuild-log-* or not
	...
	// somehow, the init task message does not exist within the /workspace/.devtool/prebuild-log-*
...
// checkFolderPermission checks the folder UID and GID is devtool
...
// checkGitFolderPermission checks the files/folders UID and GID under .git/ is devtool
