...
import (
	...
	content_service_api "github.com/khulnasoft/devtool/content-service/api"
	"github.com/khulnasoft/devtool/test/pkg/integration"
)

var (
	devtoolBuiltinUserID = "00000000-0000-0000-0000-000000000000"
)
...
func TestUploadUrl(t *testing.T) {
	tests := []struct {
		...
		{
			Name:         "simple name",
			InputOwnerID: devtoolBuiltinUserID,
			InputName:    "test-blob",
		},
		{
			Name:         "new user",
...
func TestUploadDownloadBlob(t *testing.T) {
	f := features.New("UploadDownloadBlob").
		...
		Assess("it should upload and download blob", func(testCtx context.Context, t *testing.T, cfg *envconf.Config) context.Context {
			...
			resp, err := bs.UploadUrl(ctx, &content_service_api.UploadUrlRequest{OwnerId: devtoolBuiltinUserID, Name: "test-blob"})
			if err != nil {
				t.Fatal(err)
			}
			...
			resp2, err := bs.DownloadUrl(ctx, &content_service_api.DownloadUrlRequest{OwnerId: devtoolBuiltinUserID, Name: "test-blob"})
			if err != nil {
				t.Fatal(err)
			}
