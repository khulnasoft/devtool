...
import (
	...
	"github.com/khulnasoft/devtool/installer/pkg/components"
	...
	"github.com/khulnasoft/devtool/installer/pkg/postprocess"
...
var renderCmd = &cobra.Command{
	Use:   "render",
	Short: "Renders the Kubernetes manifests required to install Devtool",
	Long: `Renders the Kubernetes manifests required to install Devtool

A config file is required which can be generated with the init command.`,
	Example: `  # Default install.
  devtool-installer render --config config.yaml | kubectl apply -f -

  # Install Devtool into a non-default namespace.
  devtool-installer render --config config.yaml --namespace devtool | kubectl apply -f -`,
...
func init() {
	...
	renderCmd.PersistentFlags().StringVarP(&renderOpts.ConfigFN, "config", "c", getEnvvar("DEVTOOL_INSTALLER_CONFIG", filepath.Join(dir, "devtool.config.yaml")), "path to the config file, use - for stdin")
