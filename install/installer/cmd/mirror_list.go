...
image to the "target" repo`,
	Example: `
  devtool-installer mirror list --config config.yaml > mirror.json

  # Pull original and push to target
  for row in $(devtool-installer mirror list --config ./config.yaml | jq -c '.[]'); do
...
func init() {
	...
	mirrorListCmd.Flags().BoolVar(&mirrorListOpts.ExcludeThirdParty, "exclude-third-party", false, "exclude non-Devtool images")
	mirrorListCmd.Flags().StringVarP(&mirrorListOpts.ConfigFN, "config", "c", os.Getenv("DEVTOOL_INSTALLER_CONFIG"), "path to the config file")
...
func generateMirrorList(cfgVersion string, cfg *configv1.Config) ([]mirrorListRepo, error) {
	// Throw error if set to the default Devtool repository
	if cfg.Repository == common.DevtoolContainerRegistry {
		return nil, fmt.Errorf("cannot mirror images to repository %s", common.DevtoolContainerRegistry)
	}

	// Get the target repository from the config
	targetRepo := strings.TrimRight(cfg.Repository, "/")

	// Use the default Devtool registry to pull from
	cfg.Repository = common.DevtoolContainerRegistry
	...
	for _, img := range rawImages {
		...
		if img == common.DevtoolContainerRegistry {
			continue
		}
		// Ignore empty image
		if img == "" {
...
// getGenericImages this is a bit brute force - anything starting "docker.io" or with Devtool repo is found
// this will be in ConfigMaps and could be anything, so will need cleaning up
func getGenericImages(k8sObj string) []string {
	var images []string

	// Search for anything that matches docker.io or the Devtool repo - docker.io needed for devtool/workspace-full
	re := regexp.MustCompile(fmt.Sprintf("%s(.*)|%s(.*)", "docker.io", common.DevtoolContainerRegistry))
