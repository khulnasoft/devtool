...
- A [Kubernetes cluster configured](https://www.devtool.io/docs/self-hosted/latest)
- A [TLS certificate](#tls-certificates)

Or, [open a Devtool workspace](https://devtool.io/from-referrer/)

The process to install Devtool is:
...
Releases can be downloaded from [GitHub Releases](https://github.com/khulnasoft/devtool/releases/)
Select your desired binary, download and install

1. Download the latest release with the command:

```shell
curl -fsSLO https://github.com/khulnasoft/devtool/releases/latest/download/devtool-installer-linux-amd64
```
...
```shell
curl -fsSLO https://github.com/khulnasoft/devtool/releases/latest/download/devtool-installer-linux-amd64.sha256
```

Validate the binary against the checksum file:

```shell
echo "$(<devtool-installer-linux-amd64.sha256)" | sha256sum --check
```

If valid, the output is:

```shell:
devtool-installer-linux-amd64: OK
```

3. Install the binary

```shell
sudo install -o root -g root devtool-installer-linux-amd64 /usr/local/bin/devtool-installer
```

4. Test to ensure the version you installed is up-to-date:

```shell
devtool-installer version
```
...
devtool-installer validate config --config devtool.config.yaml
```
...
```shell
# Checks that your cluster is ready to install Devtool
devtool-installer validate cluster --kubeconfig ~/.kube/config --config devtool.config.yaml
```
...
After a few minutes, your Devtool installation will be available on the
specified `domain`.

## Uninstallation

The Installer generates a ConfigMap with the metadata of every Kubernetes
object generated by the Installer. This can be retrieved to remove Devtool
from your cluster.

```shell
kubectl get configmaps devtool-app -o jsonpath='{.data.app\.yaml}' \
...
The Devtool Installer is designed as a way of providing you a robust and well-tested
framework for installing Devtool to your own infrastructure. There may be times when
...
```shell
error: error validating "devtool.yaml": error validating data: ValidationError(StatefulSet.status): missing required field "availableReplicas" in io.k8s.api.apps.v1.StatefulSetStatus; if you choose to ignore these errors, turn validation off with --validate=false
```
...
OpenVSX proxy (see [#8529](https://github.com/khulnasoft/devtool/issues/8529)).
...
| `kind` | Y | Installation type to run - for most users, this will be `Full` | Available options are: <ul><li>`Meta`: To install the tools that make up the front-end facing side of `Devtool` </li><li>`Workspace`: To install the components that make up the `Devtool Workspaces`</li><li>`Full`: To install the complete setup, i.e. both `Meta` and `Workspace`</li> |
...
Devtool must be connected to a Git provider. This can be done via the
...
Devtool requires certain services for it to function correctly. The Installer
...
Devtool is compatible with any registry that implements the [Docker Registry HTTP API V2](https://docs.docker.com/registry/spec/api/)
...
To configure Devtool to use Amazon, you will need to use the in-cluster
...
Devtool requires an instance of MySQL 5.7 for data storage.
...
Devtool supports the following object storage providers:
...
- `devtool.io/workload_meta`
- `devtool.io/workload_ide`
- `devtool.io/workload_services`
- `devtool.io/workload_workspace_regular`
- `devtool.io/workload_workspace_headless`
...
`ClusterIssuer` called `devtool-issuer` - please change this if necessary.

This certificate is called `https-certificates` - please use that in your
Devtool installer [config](#config).
...
By default, Devtool will be installed to the `default`
[Kubernetes namespace](https://kubernetes.io/docs/concepts/overview/working-with-objects/namespaces).
To install to a different namespace, pass a `namespace` flag to the `render`
command.

```shell
devtool-installer render --config devtool.config.yaml --namespace devtool > devtool.yaml
```
...
```shell
devtool-installer validate cluster --kubeconfig ~/.kube/config --config devtool.config.yaml --namespace devtool
```

**IMPORTANT**: this does not create the namespace, so you will need to create
that separately. This is so that uninstallation of Devtool does not remove any
Kubernetes objects, such as your TLS certificate or connection secrets.

```shell
kubectl create namespace devtool
```
