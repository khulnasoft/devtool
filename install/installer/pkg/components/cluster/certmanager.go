...
func certmanager(ctx *common.RenderContext) ([]runtime.Object, error) {
	issuerName := "devtool-self-signed-issuer"
	secretCAName := "devtool-identity-trust-root"

	devtoolCaBundleSources := []trust.BundleSource{
		{
	...
	devtoolCustomCertificateBundleSource := []trust.BundleSource{}

	if ctx.Config.CustomCACert != nil {
		devtoolCaBundleSources = append(devtoolCaBundleSources, trust.BundleSource{
			Secret: &trust.SourceObjectKeySelector{
		...
		devtoolCustomCertificateBundleSource = append(devtoolCustomCertificateBundleSource, trust.BundleSource{
			Secret: &trust.SourceObjectKeySelector{
				Name:        ctx.Config.CustomCACert.Name,
				KeySelector: trust.KeySelector{Key: "ca.crt"},
			},
	...
	objects := []runtime.Object{
		...
		&v1.Certificate{
			...
			Spec: v1.CertificateSpec{
				...
				CommonName: "root.devtool.cluster.local",
		...
		&v1.Certificate{
			...
			Spec: v1.CertificateSpec{
				...
				CommonName: "ca.devtool.cluster.local",
		...
		&trust.Bundle{
			...
			ObjectMeta: metav1.ObjectMeta{
				Name: "devtool-ca-bundle",
			},
			Spec: trust.BundleSpec{
				Sources: devtoolCaBundleSources,
		...
		// single devtool Bundle (used by registry-facade)
		&trust.Bundle{
			TypeMeta: common.TypeMetaBundle,
			ObjectMeta: metav1.ObjectMeta{
				Name: "devtool-ca",
	...
	if ctx.Config.CustomCACert != nil {
		objects = append(objects,
			...
			&trust.Bundle{
				TypeMeta: common.TypeMetaBundle,
				ObjectMeta: metav1.ObjectMeta{
					Name: "devtool-customer-certificate-bundle",
				},
				Spec: trust.BundleSpec{
					Sources: devtoolCustomCertificateBundleSource,
					Target: trust.BundleTarget{
						ConfigMap: &trust.KeySelector{
							Key: "ca-certificates.crt",
						},
