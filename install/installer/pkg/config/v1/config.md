...
Config defines the v1 version structure of the devtool config file
...
|`repository`|string|Y|  ||
|`observability.logLevel`|string|N| `trace`, `debug`, `info`, `warning`, `error`, `fatal`, `panic` |Taken from github.com/khulnasoft/devtool/components/devtool-protocol/src/util/logging.ts|
...
|`apiVersion`|string|Y|  |API version of the Devtool config defintion. `v1` in this version of Config|
...
|`experimental.webapp.publicUrl`|string|N|  |  PublicURL lets you override the publically reachable endpoints of devtool (currently only public api endpoint)  If not set, default will be api.${Domain}|
