...
func DefaultEnv(cfg *config.Config) []corev1.EnvVar {
	...
	return MergeEnv(
		[]corev1.EnvVar{
			{Name: "DEVTOOL_DOMAIN", Value: cfg.Domain},
			{Name: "DEVTOOL_INSTALLATION_SHORTNAME", Value: cfg.Metadata.InstallationShortname},
			{Name: "DEVTOOL_REGION", Value: cfg.Metadata.Region},
...
func AnalyticsEnv(cfg *config.Config) (res []corev1.EnvVar) {
	...
	return []corev1.EnvVar{{
		Name:  "DEVTOOL_ANALYTICS_WRITER",
		Value: cfg.Analytics.Writer,
	}, {
		Name:  "DEVTOOL_ANALYTICS_SEGMENT_KEY",
		Value: cfg.Analytics.SegmentKey,
	}, {
		Name:  "DEVTOOL_ANALYTICS_SEGMENT_ENDPOINT",
...
// If config registry is set to the Devtool registry, the third-party registry is returned. If
...
func ThirdPartyContainerRepo(configRegistry string, thirdPartyRegistry string) string {
	...
	if configRegistry == DevtoolContainerRegistry {
