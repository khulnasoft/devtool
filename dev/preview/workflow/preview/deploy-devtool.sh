...
DEVTOOL_AGENT_SMITH_TOKEN="$(openssl rand -hex 30)"
DEVTOOL_AGENT_SMITH_TOKEN_HASH="$(echo -n "$DEVTOOL_AGENT_SMITH_TOKEN" | sha256sum - | tr -d '  -')"
DEVTOOL_CONTAINER_REGISTRY_URL="eu.gcr.io/devtool-dev-artifact/image-build/";
DEVTOOL_IMAGE_PULL_SECRET_NAME="image-pull-secret";
DEVTOOL_PROXY_SECRET_NAME="proxy-config-certificates";
DEVTOOL_ANALYTICS="${DEVTOOL_ANALYTICS:-}"
DEVTOOL_WORKSPACE_FEATURE_FLAGS="${DEVTOOL_WORKSPACE_FEATURE_FLAGS:-}"
DEVTOOL_WITH_DEDICATED_EMU="${DEVTOOL_WITH_DEDICATED_EMU:-false}"
PREVIEW_GCP_PROJECT="devtool-dev-preview"
...
INSTALLER_CONFIG_PATH="${INSTALLER_CONFIG_PATH:-$(mktemp "/tmp/XXXXXX.devtool.config.yaml")}"
...
if ! test -f "/tmp/versions.yaml"; then
  ec=0
  docker run --rm "eu.gcr.io/devtool-dev-artifact/build/versions:$VERSION" cat /versions.yaml > /tmp/versions.yaml || ec=$?
  if [[ ec -ne 0 ]];then
...
function copyCachedCertificate {
  DESTINATION_CERT_NAME="$DEVTOOL_PROXY_SECRET_NAME"
...
#
yq w -i "${INSTALLER_CONFIG_PATH}" certificate.name "${DEVTOOL_PROXY_SECRET_NAME}"
yq w -i "${INSTALLER_CONFIG_PATH}" containerRegistry.inCluster "false"

yq w -i "${INSTALLER_CONFIG_PATH}" containerRegistry.external.url "${DEVTOOL_CONTAINER_REGISTRY_URL}"
yq w -i "${INSTALLER_CONFIG_PATH}" containerRegistry.external.certificate.kind secret
yq w -i "${INSTALLER_CONFIG_PATH}" containerRegistry.external.certificate.name "${DEVTOOL_IMAGE_PULL_SECRET_NAME}"

#
# configureDomain
#
DOMAIN="${PREVIEW_NAME}.preview.devtool-dev.com"
...
if [[ "${DEVTOOL_WITH_DEDICATED_EMU}" != "true" ]]
then
  ...
  | jq -r 'to_entries | .[] | @base64'); do
      ...
      data="${data//preview.devtool-dev.com/${DOMAIN}}"
...
yq w -i "${INSTALLER_CONFIG_PATH}" 'workspace.templates.default.spec.containers.(name==workspace).env[+].name' "DEVTOOL_PREVENT_METADATA_ACCESS"
yq w -i "${INSTALLER_CONFIG_PATH}" 'workspace.templates.default.spec.containers.(name==workspace).env.(name==DEVTOOL_PREVENT_METADATA_ACCESS).value' "true"

yq w -i "${INSTALLER_CONFIG_PATH}" 'experimental.workspace.classes.g1-standard.templates.default.spec.containers[+].name' "workspace"
yq w -i "${INSTALLER_CONFIG_PATH}" 'experimental.workspace.classes.g1-standard.templates.default.spec.containers.(name==workspace).env[+].name' "DEVTOOL_PREVENT_METADATA_ACCESS"
yq w -i "${INSTALLER_CONFIG_PATH}" 'experimental.workspace.classes.g1-standard.templates.default.spec.containers.(name==workspace).env.(name==DEVTOOL_PREVENT_METADATA_ACCESS).value' "true"

yq w -i "${INSTALLER_CONFIG_PATH}" 'experimental.workspace.classes.g1-small.templates.default.spec.containers[+].name' "workspace"
yq w -i "${INSTALLER_CONFIG_PATH}" 'experimental.workspace.classes.g1-small.templates.default.spec.containers.(name==workspace).env[+].name' "DEVTOOL_PREVENT_METADATA_ACCESS"
yq w -i "${INSTALLER_CONFIG_PATH}" 'experimental.workspace.classes.g1-small.templates.default.spec.containers.(name==workspace).env.(name==DEVTOOL_PREVENT_METADATA_ACCESS).value' "true"

#
# includeAnalytics
#
if [[ "${DEVTOOL_ANALYTICS}" == "segment" ]]; then
  DEVTOOL_ANALYTICS_SEGMENT_TOKEN="$(gcloud secrets versions access latest --secret="segment-staging-write-key" --project=${PREVIEW_GCP_PROJECT})"
  if [[ -z "${DEVTOOL_ANALYTICS_SEGMENT_TOKEN}" ]]; then
    echo "DEVTOOL_ANALYTICS_SEGMENT_TOKEN is empty"
    exit 1
  fi

  yq w -i "${INSTALLER_CONFIG_PATH}" analytics.writer segment
  yq w -i "${INSTALLER_CONFIG_PATH}" analytics.segmentKey "${DEVTOOL_ANALYTICS_SEGMENT_TOKEN}"
  ...
  yq w -i "${INSTALLER_CONFIG_PATH}" experimental.webapp.proxy.analyticsPlugin.trustedSegmentKey "${DEVTOOL_ANALYTICS_SEGMENT_TOKEN}"
  yq w -i "${INSTALLER_CONFIG_PATH}" experimental.webapp.proxy.analyticsPlugin.untrustedSegmentKey "${DEVTOOL_ANALYTICS_SEGMENT_TOKEN}"

  yq w -i "${INSTALLER_CONFIG_PATH}" 'workspace.templates.default.spec.containers.(name==workspace).env[+].name' "DEVTOOL_ANALYTICS_WRITER"
  yq w -i "${INSTALLER_CONFIG_PATH}" 'workspace.templates.default.spec.containers.(name==workspace).env.(name==DEVTOOL_ANALYTICS_WRITER).value' "segment"
  yq w -i "${INSTALLER_CONFIG_PATH}" 'workspace.templates.default.spec.containers.(name==workspace).env[+].name' "DEVTOOL_ANALYTICS_SEGMENT_ENDPOINT"
  yq w -i "${INSTALLER_CONFIG_PATH}" 'workspace.templates.default.spec.containers.(name==workspace).env.(name==DEVTOOL_ANALYTICS_SEGMENT_ENDPOINT).value' "https://${DOMAIN}/analytics"

  # add to g1-standard workspace class
  yq w -i "${INSTALLER_CONFIG_PATH}" 'experimental.workspace.classes.g1-standard.templates.default.spec.containers.(name==workspace).env[+].name' "DEVTOOL_ANALYTICS_WRITER"
  yq w -i "${INSTALLER_CONFIG_PATH}" 'experimental.workspace.classes.g1-standard.templates.default.spec.containers.(name==workspace).env.(name==DEVTOOL_ANALYTICS_WRITER).value' "segment"
  yq w -i "${INSTALLER_CONFIG_PATH}" 'experimental.workspace.classes.g1-standard.templates.default.spec.containers.(name==workspace).env[+].name' "DEVTOOL_ANALYTICS_SEGMENT_ENDPOINT"
  yq w -i "${INSTALLER_CONFIG_PATH}" 'experimental.workspace.classes.g1-standard.templates.default.spec.containers.(name==workspace).env.(name==DEVTOOL_ANALYTICS_SEGMENT_ENDPOINT).value' "https://${DOMAIN}/analytics"

  # add to g1-small workspace class
  yq w -i "${INSTALLER_CONFIG_PATH}" 'experimental.workspace.classes.g1-small.templates.default.spec.containers.(name==workspace).env[+].name' "DEVTOOL_ANALYTICS_WRITER"
  yq w -i "${INSTALLER_CONFIG_PATH}" 'experimental.workspace.classes.g1-small.templates.default.spec.containers.(name==workspace).env.(name==DEVTOOL_ANALYTICS_WRITER).value' "segment"
  yq w -i "${INSTALLER_CONFIG_PATH}" 'experimental.workspace.classes.g1-small.templates.default.spec.containers.(name==workspace).env[+].name' "DEVTOOL_ANALYTICS_SEGMENT_ENDPOINT"
  yq w -i "${INSTALLER_CONFIG_PATH}" 'experimental.workspace.classes.g1-small.templates.default.spec.containers.(name==workspace).env.(name==DEVTOOL_ANALYTICS_SEGMENT_ENDPOINT).value' "https://${DOMAIN}/analytics"
else
...
for feature in ${DEVTOOL_WORKSPACE_FEATURE_FLAGS}; do
...
WITH_VM=true "$SCRIPT_PATH/post-process.sh" "${PREVIEW_NAME}" "${DEVTOOL_AGENT_SMITH_TOKEN}"
...
leeway run components:add-smith-token \
  -DTOKEN="${DEVTOOL_AGENT_SMITH_TOKEN_HASH}" \
