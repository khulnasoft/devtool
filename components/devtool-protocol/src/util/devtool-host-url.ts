...
export class DevtoolHostUrl {
    ...
    withDomainPrefix(prefix: string): DevtoolHostUrl {
        return this.with((url) => ({ host: prefix + url.host }));
    }

    withoutWorkspacePrefix(): DevtoolHostUrl {
    ...
    withoutDomainPrefix(removeSegmentsCount: number): DevtoolHostUrl {
    ...
    with(urlUpdate: UrlUpdate) {
        ...
        return new DevtoolHostUrl(result);
    }
    ...
    asWorkspaceAuth(instanceID: string): DevtoolHostUrl {
    ...
    asApiLogout(): DevtoolHostUrl {
        return this.withApi((url) => ({ pathname: "/logout/" }));
    }

    asIDEProxy(): DevtoolHostUrl {
        const hostSegments = this.url.host.split(".");
    ...
    asPublicServices(): DevtoolHostUrl {
        const hostSegments = this.url.host.split(".");
    ...
    asIDEMetrics(): DevtoolHostUrl {
        let newUrl: DevtoolHostUrl = this;
        const hostSegments = this.url.host.split(".");
        if (hostSegments[0] !== "ide") {
