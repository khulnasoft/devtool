...
const schema = require("../data/devtool-schema.json");
...
@injectable()
export class DevtoolFileParser {
    public parse(content: string, parseOptions = {}, defaultConfig: WorkspaceConfig = {}): ParseResult {
        ...
        try {
            ...
            log.error("Unparsable Devtool configuration", err, { content });
            return {
                config: defaultConfig,
                validationErrors: ["Unparsable Devtool configuration: " + err.toString()],
            };
        }
    }
}
