...
public class DevtoolServerLauncher {
    ...
    private final DevtoolClient client;

    private DevtoolServerLauncher(
    ...
    public DevtoolServerConnection listen(
            String apiUrl,
            String origin,
            String userAgent,
        ...
        String devtoolHost = URI.create(apiUrl).getHost();
        ...
        for (Proxy proxy : proxies) {
            ...
            if (!(proxyAddress instanceof InetSocketAddress)) {
                DevtoolServerConnectionImpl.LOG.log(Level.WARNING, devtoolHost + ": unexpected proxy:", proxy);
                continue;
            }
        ...
        DevtoolServerConnectionImpl connection = new DevtoolServerConnectionImpl(devtoolHost);
        connection.whenComplete((input, exception) -> {
            try {
                container.stop();
            } catch (Throwable t) {
                DevtoolServerConnectionImpl.LOG.log(Level.WARNING, devtoolHost + ": failed to stop websocket container:", t);
            }
        });
        ...
        connection.setSession(container.connectToServer(new Endpoint() {
            ...
            public void onError(Session session, Throwable thr) {
                DevtoolServerConnectionImpl.LOG.log(Level.WARNING, devtoolHost + ": connection error:", thr);
    ...
    public static DevtoolServerLauncher create(DevtoolClient client) {
        return new Builder().create(client);
    }

    private static class Builder extends Launcher.Builder<DevtoolServer> {

        public DevtoolServerLauncher create(DevtoolClient client) {
            setLocalService(client);
            setRemoteInterface(DevtoolServer.class);
            ...
            return new DevtoolServerLauncher(
                    messageReader,
                    jsonHandler,
                    remoteEndpoint,
                    messageWriter,
