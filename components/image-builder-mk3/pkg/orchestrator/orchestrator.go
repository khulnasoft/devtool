...
import (
	...
	"github.com/khulnasoft/devtool/image-builder/api"
	protocol "github.com/khulnasoft/devtool/image-builder/api"
	"github.com/khulnasoft/devtool/image-builder/api/config"
	"github.com/khulnasoft/devtool/image-builder/pkg/auth"
...
func (o *Orchestrator) Build(req *protocol.BuildRequest, resp protocol.ImageBuilder_BuildServer) (err error) {
		...
		swr, err = o.wsman.StartWorkspace(ctx, &wsmanapi.StartWorkspaceRequest{
			...
			Spec: &wsmanapi.StartWorkspaceSpec{
				...
				Envvars: []*wsmanapi.EnvironmentVariable{
					...
					{Name: "DEVTOOL_TASKS", Value: `[{"name": "build", "init": "sudo -E /app/bob build"}]`},
	...
	for {
		...
		// "cannot pull from reg.devtool.io" error message. Instead the image-build should fail properly.
