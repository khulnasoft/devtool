...
import { Disposable, DisposableCollection, RunningWorkspaceInfo, WorkspaceInstance } from "@devtool/devtool-protocol";
...
export class WorkspaceInstanceControllerImpl implements WorkspaceInstanceController, Disposable {
    ...
    async onStopped(ctx: TraceContext, ownerUserID: string, instance: WorkspaceInstance): Promise<void> {
        ...
        try {
            await this.userDB.trace({ span }).deleteDevtoolTokensNamedLike(ownerUserID, `${instance.id}-%`);
