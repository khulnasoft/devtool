...
export interface UserDB extends OAuthUserRepository, OAuthTokenRepository, TransactionalDB<UserDB> {
    ...
    findUserByDevtoolToken(
        tokenHash: string,
        tokenType?: DevtoolTokenType,
    ): Promise<{ user: User; token: DevtoolToken } | undefined>;
    findDevtoolTokensOfUser(userId: string, tokenHash: string): Promise<DevtoolToken | undefined>;
    findAllDevtoolTokensOfUser(userId: string): Promise<DevtoolToken[]>;
    storeDevtoolToken(token: DevtoolToken): Promise<void>;
    deleteDevtoolToken(tokenHash: string): Promise<void>;
    deleteDevtoolTokensNamedLike(userId: string, namePattern: string): Promise<void>;
