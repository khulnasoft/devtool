...
import { DevtoolTableDescriptionProvider, TableDescriptionProvider } from "./tables";
...
export const dbContainerModule = (cacheClass = DataCacheNoop) =>
    new ContainerModule((bind, unbind, isBound, rebind, unbindAsync, onActivation, onDeactivation) => {
        ...
        bind(DevtoolTableDescriptionProvider).toSelf().inSingletonScope();
        bind(TableDescriptionProvider).toService(DevtoolTableDescriptionProvider);
