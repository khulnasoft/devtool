...
class UserDBSpec {
    ...
    public async findTokenAndOwner() {
        ...
        await this.db.storeDevtoolToken({
            tokenHash: "tokenhash",
        ...
        const result = await this.db.findUserByDevtoolToken("tokenhash");
    ...
    @test(timeout(10000))
    public async findDevtoolTokenOfUser() {
        let user1 = await this.db.newUser();
        user1.name = "ABC";
        user1.identities.push(TestData.ID1);
        user1.identities.push(TestData.ID2);
        ...
        await this.db.storeDevtoolToken(token);
        const result = await this.db.findDevtoolTokensOfUser(user1.id, token.tokenHash);
        expect(result).to.not.be.undefined;
    ...
    @test(timeout(10000))
    public async findAllDevtoolTokensOfUser() {
        let user1 = await this.db.newUser();
        user1.name = "ABC";
        user1.identities.push(TestData.ID1);
        user1.identities.push(TestData.ID2);
        ...
        await this.db.storeDevtoolToken(token2);
        const result = await this.db.findAllDevtoolTokensOfUser(user1.id);
        expect(result).to.not.be.undefined;
