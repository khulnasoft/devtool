...
import { DBDevtoolToken } from "./entity/db-devtool-token";
...
export class TypeORMUserDBImpl extends TransactionalDBImpl<UserDB> implements UserDB {
    ...
    private async getDevtoolTokenRepo(): Promise<Repository<DBDevtoolToken>> {
        return (await this.getEntityManager()).getRepository<DBDevtoolToken>(DBDevtoolToken);
    }
    ...
    public async findUserByDevtoolToken(
        tokenHash: string,
        tokenType?: DevtoolTokenType,
    ): Promise<{ user: User; token: DevtoolToken } | undefined> {
        const repo = await this.getDevtoolTokenRepo();
        const qBuilder = repo.createQueryBuilder("devtoolToken");
        if (!!tokenType) {
            qBuilder.where("devtoolToken.tokenHash = :tokenHash AND devtoolToken.type = :tokenType", {
                tokenHash,
                tokenType,
            });
        } else {
            qBuilder.where("devtoolToken.tokenHash = :tokenHash", { tokenHash });
        }
    ...
    public async findDevtoolTokensOfUser(userId: string, tokenHash: string): Promise<DevtoolToken | undefined> {
        const repo = await this.getDevtoolTokenRepo();
        const qBuilder = repo.createQueryBuilder("devtoolToken");
        qBuilder.where("userId = :userId AND devtoolToken.tokenHash = :tokenHash", { userId, tokenHash });
        return qBuilder.getOne();
    }

    public async findAllDevtoolTokensOfUser(userId: string): Promise<DevtoolToken[]> {
        const repo = await this.getDevtoolTokenRepo();
        const qBuilder = repo.createQueryBuilder("devtoolToken");
    ...
    public async storeDevtoolToken(token: DevtoolToken): Promise<void> {
        const repo = await this.getDevtoolTokenRepo();
        await repo.insert(token);
        await this.cache.invalidate(getUserCacheKey(token.userId));
    }

    public async deleteDevtoolToken(tokenHash: string): Promise<void> {
        const repo = await this.getDevtoolTokenRepo();
        await repo.delete({ tokenHash });
    }

    public async deleteDevtoolTokensNamedLike(userId: string, namePattern: string): Promise<void> {
        const repo = await this.getDevtoolTokenRepo();
    ...
    async issueRefreshToken(accessToken: OAuthToken): Promise<OAuthToken> {
        // NOTE(rl): this exists for the OAuth server code - Devtool tokens are non-refreshable (atm)
    ...
    async persist(accessToken: OAuthToken): Promise<void> {
        ...
        if (userAndToken) {
            ...
            dbToken = userAndToken.token as DevtoolToken & { user: DBUser };
            const repo = await this.getDevtoolTokenRepo();
            ...
            return this.storeDevtoolToken(dbToken);
        }
    }
    async revoke(accessTokenToken: OAuthToken): Promise<void> {
        ...
        await this.deleteDevtoolToken(tokenHash);
    }
