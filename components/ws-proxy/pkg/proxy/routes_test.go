...
import (
	...
	server_lib "github.com/khulnasoft/devtool/server/go/pkg/lib"
...
var (
	...
	config = Config{
		...
		DevtoolInstallation: &DevtoolInstallation{
	...
	tests := []struct {
		...
		},
		{
			...
			Request: modifyRequest(httptest.NewRequest("GET", workspaces[0].URL+"somewhere/in/the/ide", nil),
				addHostHeader,
				addOwnerToken(domain, workspaces[0].InstanceID, workspaces[0].Auth.OwnerToken),
				addHeader("Origin", config.DevtoolInstallation.HostName),
...
func TestRemoveSensitiveCookies(t *testing.T) {
	var (
		...
		realDevtoolSessionCookie = &http.Cookie{Domain: domain, Name: server_lib.CookieNameFromDomain(domain), Value: "fobar"}
	...
	tests := []struct {
		...
		{Name: "real Devtool session cookie", Input: []*http.Cookie{realDevtoolSessionCookie, miscCookie}, Expected: []*http.Cookie{miscCookie}},
