...
import (
	...
	"github.com/khulnasoft/devtool/devtool-cli/pkg/supervisor"
...
func runRebuild(ctx context.Context, supervisorClient *supervisor.SupervisorClient) error {
	...
	devtoolConfig, err := utils.ParseDevtoolConfig(checkoutLocation)
	if err != nil {
		fmt.Println("The .devtool.yml file cannot be parsed: please check the file and try again")
		fmt.Println("")
		fmt.Println("For help check out the reference page:")
		fmt.Println("https://www.devtool.io/docs/references/devtool-yml#devtoolyml")
		return GpError{Err: err, OutCome: utils.Outcome_UserErr, ErrorCode: utils.RebuildErrorCode_MalformedDevtoolYaml, Silence: true}
	}

	if devtoolConfig == nil {
		fmt.Println("To test the image build, you need to configure your project with a .devtool.yml file")
		fmt.Println("")
		...
		fmt.Println("https://www.devtool.io/docs/configure#configure-devtool")
		return GpError{Err: err, OutCome: utils.Outcome_UserErr, ErrorCode: utils.RebuildErrorCode_MissingDevtoolYaml, Silence: true}
	}

	var image string
	var dockerfilePath string
	var dockerContext string
	switch img := devtoolConfig.Image.(type) {
	case nil:
		...
		if image == "" {
			image = "devtool/workspace-full:latest"
		}
	...
	case map[interface{}]interface{}:
		...
		if _, err := os.Stat(dockerfilePath); os.IsNotExist(err) {
			fmt.Println("Your .devtool.yml points to a Dockerfile that doesn't exist: " + dockerfilePath)
		...
		if string(dockerfile) == "" {
			fmt.Println("Your Devtool's Dockerfile is empty")
			fmt.Println("")
			fmt.Println("To learn how to customize your workspace, check out the following docs:")
			fmt.Println("https://www.devtool.io/docs/configure/workspaces/workspace-image#use-a-custom-dockerfile")
			fmt.Println("")
			fmt.Println("Once you configure your Dockerfile, re-run this command to validate your changes")
			return GpError{Err: err, OutCome: utils.Outcome_UserErr, Silence: true}
		}
	default:
		fmt.Println("Check your .devtool.yml and make sure the image property is configured correctly")
		return GpError{Err: err, OutCome: utils.Outcome_UserErr, ErrorCode: utils.RebuildErrorCode_MalformedDevtoolYaml, Silence: true}
	}
	...
	if err != nil {
		dockerPath = "/.supervisor/devtool-docker-cli"
	}
	...
	workspaceLocation := devtoolConfig.WorkspaceLocation
	...
	tasks, err := json.Marshal(devtoolConfig.Tasks)
	if err != nil {
		return err
	}
	...
	for _, env := range validateOpts.DevtoolEnvs {
		envs += env + "\n"
	}
	if validateOpts.Headless {
		envs += "DEVTOOL_HEADLESS=true\n"
	}
	...
	prepareFS := []mnte{
		...
		{Source: "/workspace/.devtool-debug/.docker-root", Target: "/workspace/.docker-root", Permission: 0710},
		{Source: "/workspace/.devtool-debug/.devtool", Target: "/workspace/.devtool", Permission: 0751},
		{Source: "/workspace/.devtool-debug/.vscode-remote", Target: "/workspace/.vscode-remote", Permission: 0751},
		{Source: "/workspace/.devtool-debug/.cache", Target: "/workspace/.cache", Permission: 0751},
		{Source: "/workspace/.devtool-debug/.config", Target: "/workspace/.config", Permission: 0751},
	...
	for _, mnt := range prepareFS {
		...
		if mnt.Target == "" {
			...
			// if target is not same with source and it not a file, ensure target is created by devtool user
...
Connect using SSH keys (https://devtool.io/keys):
%s
...
func init() {
	...
	setFlags := func(cmd *cobra.Command) {
		...
		// internal
		cmd.PersistentFlags().StringArrayVarP(&validateOpts.DevtoolEnvs, "devtool-env", "", nil, "")
		...
		_ = cmd.PersistentFlags().MarkHidden("devtool-env")
