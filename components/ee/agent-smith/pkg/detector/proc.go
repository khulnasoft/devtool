...
func (p realProcfs) Environ(pid int) ([]string, error) {
	...
	return parseDevtoolEnviron(f)
}

func parseDevtoolEnviron(r io.Reader) ([]string, error) {
	// Note: this function is benchmarked in BenchmarkParseDevtoolEnviron.
	//       At the time of this wriging it consumed 3+N allocs where N is the number of
	//       env vars starting with DEVTOOL_.
	//
	...
	for scan.Scan() {
		// we only keep DEVTOOL_ variables for optimisation
		text := scan.Bytes()
		if !bytes.HasPrefix(text, []byte("DEVTOOL_")) {
			continue
		}
...
func extractWorkspaceFromWorkspacekit(proc discoverableProcFS, pid int) *common.Workspace {
	...
	for _, e := range env {
		if strings.HasPrefix(e, "DEVTOOL_OWNER_ID=") {
			ownerID = strings.TrimPrefix(e, "DEVTOOL_OWNER_ID=")
			continue
		}
		if strings.HasPrefix(e, "DEVTOOL_WORKSPACE_ID=") {
			workspaceID = strings.TrimPrefix(e, "DEVTOOL_WORKSPACE_ID=")
			continue
		}
		if strings.HasPrefix(e, "DEVTOOL_INSTANCE_ID=") {
			instanceID = strings.TrimPrefix(e, "DEVTOOL_INSTANCE_ID=")
			continue
		}
		if strings.HasPrefix(e, "DEVTOOL_WORKSPACE_CONTEXT_URL=") {
			gitURL = strings.TrimPrefix(e, "DEVTOOL_WORKSPACE_CONTEXT_URL=")
			continue
		}
	}
