...
import (
	...
	"github.com/khulnasoft/devtool/agent-smith/pkg/classifier"
	"github.com/khulnasoft/devtool/agent-smith/pkg/common"
	"github.com/khulnasoft/devtool/agent-smith/pkg/config"
	"github.com/khulnasoft/devtool/agent-smith/pkg/detector"
...
type Smith struct {
	...
	DevtoolAPI        devtool.APIInterface
...
func NewAgentSmith(cfg config.Config) (*Smith, error) {
	...
	var api devtool.APIInterface
	if cfg.DevtoolAPI.HostURL != "" {
		u, err := url.Parse(cfg.DevtoolAPI.HostURL)
		if err != nil {
			return nil, xerrors.Errorf("cannot parse Devtool API host url: %w", err)
		}
		endpoint := fmt.Sprintf("wss://%s/api/v1", u.Hostname())

		api, err = devtool.ConnectToServer(endpoint, devtool.ConnectToServerOpts{
			Context: context.Background(),
			Token:   cfg.DevtoolAPI.APIToken,
			Log:     log.Log,
		})
		if err != nil {
			return nil, xerrors.Errorf("cannot connect to Devtool API: %w", err)
		}
	}
