...
type APIInterface interface {
	GetToken(ctx context.Context, query *devtool.GetTokenSearchOptions) (res *devtool.Token, err error)
	OpenPort(ctx context.Context, port *devtool.WorkspaceInstancePort) (res *devtool.WorkspaceInstancePort, err error)
	UpdateGitStatus(ctx context.Context, status *devtool.WorkspaceInstanceRepoStatus) (err error)
	WorkspaceUpdates(ctx context.Context) (<-chan *devtool.WorkspaceInstance, error)
...
const (
	// KindDevtool marks tokens that provide access to the Devtool server API.
	KindDevtool = "devtool"
)
...
type Service struct {
	...
	subs     map[chan *devtool.WorkspaceInstance]struct{}
	subMutex sync.Mutex
...
func NewServerApiService(ctx context.Context, cfg *ServiceConfig, tknsrv api.TokenServiceServer) *Service {
	...
	if err != nil {
		log.WithError(err).Error("cannot get token for Devtool API")
		return nil
	}
	...
	service := &Service{
		...
		subs:       make(map[chan *devtool.WorkspaceInstance]struct{}),
	}
...
func (s *Service) GetToken(ctx context.Context, query *devtool.GetTokenSearchOptions) (res *devtool.Token, err error) {
	if s == nil {
...
func (s *Service) UpdateGitStatus(ctx context.Context, status *devtool.WorkspaceInstanceRepoStatus) (err error) {
	if s == nil {
		return errNotConnected
	}
	startTime := time.Now()
...
func (s *Service) OpenPort(ctx context.Context, port *devtool.WorkspaceInstancePort) (res *devtool.WorkspaceInstancePort, err error) {
	if s == nil {
		return nil, errNotConnected
	}
	startTime := time.Now()
	...
	// see https://github.com/khulnasoft/devtool/blob/2967579c330de67090d975661a6e3e1cd970ab68/components/server/src/workspace/devtool-server-impl.ts#L1521
	return port, nil
}
...
func (s *Service) WorkspaceUpdates(ctx context.Context) (<-chan *devtool.WorkspaceInstance, error) {
	if s == nil {
		return nil, errNotConnected
	}
	ch := make(chan *devtool.WorkspaceInstance)
	s.subMutex.Lock()
	s.subs[ch] = struct{}{}
	s.subMutex.Unlock()
...
func workspaceStatusToWorkspaceInstance(status *v1.WorkspaceStatus) *devtool.WorkspaceInstance {
	instance := &devtool.WorkspaceInstance{
		CreationTime: status.Instance.CreatedAt.String(),
		ID:           status.Instance.InstanceId,
		Status: &devtool.WorkspaceInstanceStatus{
			ExposedPorts: []*devtool.WorkspaceInstancePort{},
	...
	for _, port := range status.Instance.Status.Ports {
		info := &devtool.WorkspaceInstancePort{
			Port: float64(port.Port),
			URL:  port.Url,
		}
		if port.Policy == v1.PortPolicy_PORT_POLICY_PUBLIC {
			info.Visibility = devtool.PortVisibilityPublic
		} else {
			info.Visibility = devtool.PortVisibilityPrivate
		}
		if port.Protocol == v1.PortProtocol_PORT_PROTOCOL_HTTPS {
			info.Protocol = devtool.PortProtocolHTTPS
		} else {
			info.Protocol = devtool.PortProtocolHTTP
		}
