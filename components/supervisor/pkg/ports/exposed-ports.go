...
// DevtoolExposedPorts uses a connection to the Devtool server to implement
// the ExposedPortsInterface.
type DevtoolExposedPorts struct {
	WorkspaceID   string
	InstanceID    string
	WorkspaceUrl  string
	...
	lastServerExposed  []*devtool.WorkspaceInstancePort

	requests chan *exposePortRequest
}

type exposePortRequest struct {
	port *devtool.WorkspaceInstancePort
	ctx  context.Context
	done chan error
}

// NewDevtoolExposedPorts creates a new instance of DevtoolExposedPorts
func NewDevtoolExposedPorts(workspaceID string, instanceID string, workspaceUrl string, devtoolService serverapi.APIInterface) *DevtoolExposedPorts {
	return &DevtoolExposedPorts{
...
func (g *DevtoolExposedPorts) getPortUrl(port uint32) string {
...
func (g *DevtoolExposedPorts) getPortProtocol(protocol string) string {
	switch protocol {
	case devtool.PortProtocolHTTP, devtool.PortProtocolHTTPS:
		return protocol
	default:
		return devtool.PortProtocolHTTP
	}
}

func (g *DevtoolExposedPorts) existInLocalExposed(port uint32) bool {
...
// Observe starts observing the exposed ports until the context is canceled.
func (g *DevtoolExposedPorts) Observe(ctx context.Context) (<-chan []ExposedPort, <-chan error) {
	var (
	...
	go func() {
		...
		updates, err := g.devtoolService.WorkspaceUpdates(ctx)
		if err != nil {
			errchan <- err
			return
		}
		mixin := func(localExposedPort []uint32, serverExposePort []*devtool.WorkspaceInstancePort) []ExposedPort {
			...
			for _, port := range g.localExposedPort {
				res[port] = ExposedPort{
					...
					Protocol:  devtool.PortProtocolHTTP,
				}
			}
...
func (g *DevtoolExposedPorts) Run(ctx context.Context) {
...
func (g *DevtoolExposedPorts) doExpose(req *exposePortRequest) {
	var err error
	defer func() {
		if err != nil {
			req.done <- err
	...
	attempt := 0
	for {
		_, err = g.devtoolService.OpenPort(req.ctx, req.port)
...
// Expose exposes a port to the internet. Upon successful execution any Observer will be updated.
func (g *DevtoolExposedPorts) Expose(ctx context.Context, local uint32, public bool, protocol string) <-chan error {
	if protocol != devtool.PortProtocolHTTPS && protocol != devtool.PortProtocolHTTP {
		protocol = devtool.PortProtocolHTTP
	}
	if !public && protocol != devtool.PortProtocolHTTPS {
	...
	visibility := devtool.PortVisibilityPrivate
	if public {
		visibility = devtool.PortVisibilityPublic
	}
