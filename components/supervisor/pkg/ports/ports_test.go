...
func TestPortsUpdateState(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			Changes: []Change{
				{
					Config: &ConfigChange{instance: []*devtool.PortsItems{
		...
		{
			Desc: "port status has the name attribute set as soon as the port gets exposed, if there was a name configured in Devtool's Workspace",
			Changes: []Change{
				{
					Config: &ConfigChange{instance: []*devtool.PortsItems{
		...
		},
		{
			...
			// see also https://github.com/khulnasoft/openvscode-server/blob/5ab7644a8bbf37d28e23212bc6f1529cafd8bf7b/extensions/devtool-web/src/extension.ts#L310-L339
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, port := range tt.fields.orderInYaml {
				portsItems = append(portsItems, &devtool.PortsItems{Port: port})
			}
