...
func (service *ConfigService) update(config *devtool.DevtoolConfig, current *Configs) bool {
	currentPortConfigs, currentRangeConfigs := current.instancePortConfigs, current.instanceRangeConfigs
	var ports []*devtool.PortsItems
	if config != nil {
		ports = config.Ports
	}
...
func parseInstanceConfigs(ports []*devtool.PortsItems) (portConfigs map[uint32]*SortConfig, rangeConfigs []*RangeConfig) {
