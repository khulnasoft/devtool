...
type GitTokenProvider struct {
	...
	devtoolAPI           serverapi.APIInterface
}

// NewGitTokenProvider creates a new instance of gitTokenProvider.
func NewGitTokenProvider(devtoolAPI serverapi.APIInterface, workspaceConfig WorkspaceConfig, notificationService *NotificationService) *GitTokenProvider {
	return &GitTokenProvider{
		notificationService: notificationService,
		workspaceConfig:     workspaceConfig,
		devtoolAPI:           devtoolAPI,
	}
}
...
func (p *GitTokenProvider) GetToken(ctx context.Context, req *api.GetTokenRequest) (tkn *Token, err error) {
	...
	token, err := p.devtoolAPI.GetToken(ctx, &devtool.GetTokenSearchOptions{
		Host: req.Host,
	})
	if err != nil {
		return nil, err
...
func (p *GitTokenProvider) openAccessControl() error {
	...
	gpCmd := exec.Command(gpPath, "preview", "--external", p.workspaceConfig.DevtoolHost+"/access-control")
	runAsDevtoolUser(gpCmd)
...
type gitStatusUpdateContext struct {
	lastSuccessfullStatus *devtool.WorkspaceInstanceRepoStatus
...
func (s *GitStatusService) Run(ctx context.Context, wg *sync.WaitGroup) {
	...
	go func() {
		updates, err := s.devtoolService.WorkspaceUpdates(ctx)
		if err != nil {
...
func (s *GitStatusService) update(ctx context.Context, updateContext *gitStatusUpdateContext) {
	...
	var newStatus *devtool.WorkspaceInstanceRepoStatus
	if status != nil {
		...
		newStatus = &devtool.WorkspaceInstanceRepoStatus{
	...
	err = s.devtoolService.UpdateGitStatus(ctx, newStatus)
	if err != nil {
