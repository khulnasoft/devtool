...
//   2. IDE: Devtool supports different IDEs, all of which have different configuration needs.
//   3. DesktopIDE: Devtool supports to connect external IDEs (like desktop IDEs).
...
type WorkspaceConfig struct {
	...
	WorkspaceContextURL string `env:"DEVTOOL_WORKSPACE_CONTEXT_URL"`

	// WorkspaceUrl is an URL for which workspace is accessed.
	WorkspaceUrl string `env:"DEVTOOL_WORKSPACE_URL"`

	// WorkspaceClass denotes the class of the workspace
	WorkspaceClass string `env:"DEVTOOL_WORKSPACE_CLASS"`

	// WorkspaceClassInfo denotes the detail of workspace class
	WorkspaceClassInfo *WorkspaceClassInfo `env:"DEVTOOL_WORKSPACE_CLASS_INFO"`

	// DefaultWorkspaceImage is the default image of current workspace
	DefaultWorkspaceImage string `env:"DEVTOOL_DEFAULT_WORKSPACE_IMAGE"`
	...
	RepoRoot string `env:"DEVTOOL_REPO_ROOT"`
	...
	RepoRoots string `env:"DEVTOOL_REPO_ROOTS"`
	...
	PreventMetadataAccess bool `env:"DEVTOOL_PREVENT_METADATA_ACCESS"`
	...
	GitUsername string `env:"DEVTOOL_GIT_USER_NAME"`
	// GitEmail makes supervisor configure the global user.email Git setting.
	GitEmail string `env:"DEVTOOL_GIT_USER_EMAIL"`

	// Tokens is a JSON encoded list of WorkspaceDevtoolToken
	Tokens string `env:"THEIA_SUPERVISOR_TOKENS"`

	// WorkspaceID is the ID of the workspace
	WorkspaceID string `env:"DEVTOOL_WORKSPACE_ID"`

	// WorkspaceInstanceID is the instance ID of the workspace
	WorkspaceInstanceID string `env:"DEVTOOL_INSTANCE_ID"`

	// DevtoolHost points to the Devtool API server we're to talk to
	DevtoolHost string `env:"DEVTOOL_HOST"`

	// DevtoolTasks is the task configuration of the workspace
	DevtoolTasks string `env:"DEVTOOL_TASKS"`

	// DevtoolHeadless controls whether the workspace is running headless
	DevtoolHeadless string `env:"DEVTOOL_HEADLESS"`
	...
	WorkspaceContext string `env:"DEVTOOL_WORKSPACE_CONTEXT"`

	// WorkspaceClusterHost is a host under which this workspace is served, e.g. ws-eu11.devtool.io
	WorkspaceClusterHost string `env:"DEVTOOL_WORKSPACE_CLUSTER_HOST"`
	...
	TerminationGracePeriodSeconds *int `env:"DEVTOOL_TERMINATION_GRACE_PERIOD_SECONDS"`

	// OwnerId is the user id who owns the workspace
	OwnerId string `env:"DEVTOOL_OWNER_ID"`
	...
	ConfigcatEnabled bool `env:"DEVTOOL_CONFIGCAT_ENABLED"`

	SSHGatewayCAPublicKey string `env:"DEVTOOL_SSH_CA_PUBLIC_KEY"`
}

// WorkspaceDevtoolToken is a list of tokens that should be added to supervisor's token service.
type WorkspaceDevtoolToken struct {
	api.SetTokenRequest
	TokenOTS string `json:"tokenOTS"`
}

// TaskConfig defines devtool task shape.
...
func (c WorkspaceConfig) Validate() error {
	if !(0 < c.IDEPort && c.IDEPort <= math.MaxUint16) {
		return xerrors.Errorf("DEVTOOL_THEIA_PORT must be between 0 and %d", math.MaxUint16)
	}
	...
	if _, _, err := c.DevtoolAPIEndpoint(); err != nil {
		return err
	}
...
// GetTokens parses tokens from DEVTOOL_TOKENS and possibly downloads OTS.
func (c WorkspaceConfig) GetTokens(downloadOTS bool) ([]WorkspaceDevtoolToken, error) {
	if c.Tokens == "" {
		return nil, nil
	}

	var tks []WorkspaceDevtoolToken
...
// DevtoolAPIEndpoint produces the data required to connect to the Devtool API.
func (c WorkspaceConfig) DevtoolAPIEndpoint() (endpoint, host string, err error) {
	gphost, err := url.Parse(c.DevtoolHost)
	if err != nil {
		return
	}
...
func (c WorkspaceConfig) isPrebuild() bool {
	return c.DevtoolHeadless == "true" || c.DebugWorkspaceType == api.DebugWorkspaceType_prebuild
}

// getDevtoolTasks returns true if the workspace is headless.
func (c WorkspaceConfig) isHeadless() bool {
	return c.DevtoolHeadless == "true"
}
...
// getDevtoolTasks parses devtool tasks.
func (c Config) getDevtoolTasks() (tasks []TaskConfig, err error) {
	if c.DevtoolTasks != "" {
		var configured *[]TaskConfig
		err = json.Unmarshal([]byte(c.DevtoolTasks), &configured)
		if err != nil {
...
func (c WorkspaceConfig) getCommit() (commit *devtool.Commit, err error) {
