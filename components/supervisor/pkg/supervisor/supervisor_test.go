...
func TestBuildChildProcEnv(t *testing.T) {
	withBaseline := func(i []string) []string {
		return append(i,
			...
			"HISTFILE=/workspace/.devtool/.shell_history",
	...
	tests := []struct {
		...
		},
		{
			...
			Input:       []string{"DEVTOOL_TOKENS=foobar"},
			Expectation: withBaseline(nil),
		},
		{
		...
		},
		{
			Name:  "ots",
			Input: []string{},
			OTS:   `[{"name":"foo","value":"bar"},{"name":"DEVTOOL_TOKENS","value":"foobar"}]`,
			Expectation: []string{
				"HOME=/home/devtool",
				"BROWSER=/.supervisor/browser.sh",
				"HISTFILE=/workspace/.devtool/.shell_history",
				"PROMPT_COMMAND=history -a", "SUPERVISOR_ADDR=localhost:8080", "USER=devtool", "foo=bar"},
		},
		{
			Name:  "failed ots",
			Input: []string{},
			...
			Expectation: []string{
				...
				"BROWSER=/.supervisor/browser.sh",
				"HISTFILE=/workspace/.devtool/.shell_history",
				"PROMPT_COMMAND=history -a", "SUPERVISOR_ADDR=localhost:8080", "USER=devtool"},
		},
	}
...
func TestIsBlacklistedEnvvar(t *testing.T) {
	tests := []struct {
		...
		{Name: "devtool tokens", Input: "DEVTOOL_TOKENS", Expectation: true},
		{Name: "devtool tokens child", Input: "DEVTOOL_TOKENS_GITHUB", Expectation: true},
