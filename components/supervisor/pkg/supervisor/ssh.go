...
func prepareSSHKey(ctx context.Context, sshkey string) error {
	...
	// We need to force HOME because the Devtool user might not have existed at the start of the container
	...
	err = os.Chown(sshkey, devtoolUID, devtoolGID)
	if err != nil {
...
func ensureSSHDir(cfg *Config) error {
	...
	_ = exec.Command("chown", "-R", fmt.Sprintf("%d:%d", devtoolUID, devtoolGID), d).Run()
...
func configureSSHDefaultDir(cfg *Config) {
	...
	file, err := os.OpenFile("/home/devtool/.bash_profile", os.O_WRONLY|os.O_APPEND|os.O_CREATE, 0o644)
	if err != nil {
...
func configureSSHMessageOfTheDay() {
	msg := []byte(`Welcome to Devtool: Always ready to code. Try the following commands to get started:

	gp tasks list         List all your defined tasks in .devtool.yml
...
For more information, see the Devtool documentation: https://devtool.io/docs
`)
