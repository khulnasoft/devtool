...
func AddDevtoolUserIfNotExists() error {
	ok, err := hasGroup(devtoolGroupName, devtoolGID)
	if err != nil {
		return err
	}
	if !ok {
		err = addGroup(devtoolGroupName, devtoolGID)
		if err != nil {
			return err
		}
	}
	if err := addSudoer(devtoolGroupName); err != nil {
		log.WithError(err).Error("add devtool sudoers")
	}

	targetUser := &user.User{
		Uid:      strconv.Itoa(devtoolUID),
		Gid:      strconv.Itoa(devtoolGID),
		Username: devtoolUserName,
		HomeDir:  "/home/" + devtoolUserName,
	}
...
func hasGroup(name string, gid int) (bool, error) {
	...
	if grpByID == nil && grpByName != nil {
		...
		return true, xerrors.Errorf("group named %s exists but uses different GID %s, should be: %d", name, grpByName.Gid, devtoolGID)
	}
...
func hasUser(u *user.User) (bool, error) {
	...
	if userByID == nil && userByName != nil {
		...
		return true, xerrors.Errorf("user named %s exists but uses different UID %s, should be: %d", u.Username, userByName.Uid, devtoolUID)
	}
...
func addSudoer(group string) error {
	...
	devtoolSudoer := []byte(fmt.Sprintf("%%%s ALL=NOPASSWD:ALL", group))
	// Line starts with "%devtool ..."
	re := regexp.MustCompile(fmt.Sprintf("(?m)^%%%s\\s+.*?$", group))
	if len(re.FindStringIndex(string(b))) > 0 {
		nb := re.ReplaceAll(b, devtoolSudoer)
