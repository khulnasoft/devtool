...
const (
	devtoolLogDir          = "/var/log/devtool"
	supervisorLogFilePath = devtoolLogDir + "/supervisor.log"
)
...
func initLog(json bool) io.Closer {
	...
	if err := os.MkdirAll(devtoolLogDir, 0755); err != nil {
		log.WithError(err).Error("cannot create devtool log directory")
