...
var callServerCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		api, err := devtool.ConnectToServer(fmt.Sprintf("ws://%s/api/v1", host), devtool.ConnectToServerOpts{
			Token: token,
			Log:   log.Log,
		})
		if err != nil {
...
func init() {
	...
	callServerCmd.Flags().String("workspace-id", os.Getenv("DEVTOOL_WORKSPACE_ID"), "workspace ID to listen for")
}
