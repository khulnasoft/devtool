...
import (
	...
	"github.com/khulnasoft/devtool/common-go/watch"
	devtoolapi "github.com/khulnasoft/devtool/devtool-protocol"
	api "github.com/khulnasoft/devtool/ide-service-api"
	"github.com/khulnasoft/devtool/ide-service-api/config"
...
func (s *IDEServiceServer) ResolveWorkspaceConfig(ctx context.Context, req *api.ResolveWorkspaceConfigRequest) (resp *api.ResolveWorkspaceConfigResponse, err error) {
	...
	if os.Getenv("CONFIGCAT_SDK_KEY") != "" {
		resp.Envvars = append(resp.Envvars, &api.EnvironmentVariable{
			Name:  "DEVTOOL_CONFIGCAT_ENABLED",
			Value: "true",
		})
	}

	var wsConfig *devtoolapi.DevtoolConfig
	...
	if req.Type == api.WorkspaceType_REGULAR {
		...
		if preferToolbox {
			preferToolboxEnv := api.EnvironmentVariable{
				Name:  "DEVTOOL_PREFER_TOOLBOX",
				Value: "true",
			}
			debuggingEnv := api.EnvironmentVariable{
				Name:  "DEVTOOL_TOOLBOX_DEBUGGING",
...
func getPrebuilds(config *devtoolapi.DevtoolConfig, alias string) *devtoolapi.Prebuilds {
...
func getProductConfig(config *devtoolapi.DevtoolConfig, alias string) *devtoolapi.JetbrainsProduct {
	defer func() {
	...
	productConfig, ok := v.(*devtoolapi.JetbrainsProduct)
	if !ok {
		return nil
	}
