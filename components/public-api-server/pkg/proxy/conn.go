...
type ServerConnectionPool interface {
	...
	Get(ctx context.Context, token auth.Token) (devtool.APIInterface, error)
}
...
func (p *NoConnectionPool) Get(ctx context.Context, token auth.Token) (devtool.APIInterface, error) {
	...
	opts := devtool.ConnectToServerOpts{
		Context: ctx,
		Log:     logger,
	...
	conn, err := devtool.ConnectToServer(p.ServerAPI.String(), opts)
	if err != nil {
...
func NewConnectionPool(address *url.URL, poolSize int) (*ConnectionPool, error) {
	cache, err := lru.NewWithEvict(poolSize, func(_, value interface{}) {
		...
		conn, ok := value.(devtool.APIInterface)
		if !ok {
			log.Errorf("Failed to cast cache value to devtool API Interface")
			return
		}
	...
	return &ConnectionPool{
		cache: cache,
		connConstructor: func(ctx context.Context, token auth.Token) (devtool.APIInterface, error) {
			opts := devtool.ConnectToServerOpts{
			...
			conn, err := devtool.ConnectToServer(endpoint, opts)
			if err != nil {
				return nil, fmt.Errorf("failed to create new connection to server: %w", err)
			}
...
type ConnectionPool struct {
	connConstructor func(context.Context, auth.Token) (devtool.APIInterface, error)

	// cache stores token to connection mapping
	cache *lru.Cache
}

func (p *ConnectionPool) Get(ctx context.Context, token auth.Token) (devtool.APIInterface, error) {
	origin := origin.FromContext(ctx)
	...
	if found {
		conn, ok := cached.(*devtool.APIoverJSONRPC)
		if ok {
			return conn, nil
		}
	}
...
func getEndpointBasedOnToken(t auth.Token, u *url.URL) (string, error) {
	...
	case auth.CookieTokenType:
		return fmt.Sprintf("%s/devtool", u.String()), nil
	default:
