...
import (
	...
	"github.com/khulnasoft/devtool/common-go/experiments/experimentstest"
...
func TestGetIDToken(t *testing.T) {
	const workspaceID = "devtoolio-devtool-te23l4bjejv"
	...
	}{
		{
			...
			TokenSource: func(t *testing.T) IDTokenSource {
				return functionIDTokenSource(func(ctx context.Context, org string, audience []string, userInfo oidc.UserInfo) (string, error) {
					require.Equal(t, "correct@devtool.io", userInfo.GetEmail())
					...
					&protocol.WorkspaceInfo{
						Workspace: &protocol.Workspace{
							...
							Context: &protocol.WorkspaceContext{
								...
								NormalizedContextURL: "https://github.com/khulnasoft/devtool",
							},
						},
					},
					nil,
				...
				ma.EXPECT().GetLoggedInUser(gomock.Any()).Return(
					&protocol.User{
						...
						Identities: []*protocol.Identity{
							nil,
							{Deleted: true, PrimaryEmail: "nonsense@devtool.io"},
							{Deleted: false, PrimaryEmail: "correct@devtool.io", LastSigninTime: "2021-01-01T00:00:00Z"},
						},
						OrganizationId: "test",
					},
					nil,
