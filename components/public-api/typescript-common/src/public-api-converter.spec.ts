...
import { BranchMatchingStrategy } from "@khulnasoft/public-api/lib/devtool/v1/configuration_pb";
import { AuthProviderType } from "@khulnasoft/public-api/lib/devtool/v1/authprovider_pb";
import { Workspace, WorkspacePhase_Phase } from "@khulnasoft/public-api/lib/devtool/v1/workspace_pb";
import { WorkspaceAndInstance } from "@khulnasoft/devtool-protocol";

describe("PublicAPIConverter", () => {
    const converter = new PublicAPIConverter();

    const testDevtoolHost = "https://devtool-test.preview.devtool-dev.com/";
    ...
    describe("golden tests", () => {
        ...
        it("toPrebuild", async () => {
            await startFixtureTest("../fixtures/toPrebuild_*.json", async (input) =>
                converter.toPrebuild(testDevtoolHost, input),
            );
        });
    ...
    describe("errors", () => {
        ...
        it("INVALID_DEVTOOL_YML", () => {
            const connectError = converter.toError(
                new InvalidDevtoolYMLError({
                    violations: ['Invalid value: "": must not be empty'],
                }),
            );
            expect(connectError.code).to.equal(Code.FailedPrecondition);
            expect(connectError.rawMessage).to.equal('Invalid devtool.yml: Invalid value: "": must not be empty');

            const details = connectError.findDetails(FailedPreconditionDetails)[0];
            expect(details).to.not.be.undefined;
            expect(details?.reason?.case).to.equal("invalidDevtoolYml");
            expect(details?.reason?.value).to.be.instanceOf(InvalidDevtoolYMLErrorData);

            let violations = (details?.reason?.value as InvalidDevtoolYMLErrorData).violations;
            expect(violations).to.deep.equal(['Invalid value: "": must not be empty']);

            const appError = converter.fromError(connectError);
            expect(appError).to.be.instanceOf(InvalidDevtoolYMLError);
            expect(appError.code).to.equal(ErrorCodes.INVALID_DEVTOOL_YML);
            expect(appError.message).to.equal('Invalid devtool.yml: Invalid value: "": must not be empty');

            violations = (appError as InvalidDevtoolYMLError).info.violations;
            expect(violations).to.deep.equal(['Invalid value: "": must not be empty']);
        });
