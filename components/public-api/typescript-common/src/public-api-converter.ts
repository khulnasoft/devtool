...
export class PublicAPIConverter {
    ...
    toError(reason: unknown): ConnectError {
        ...
        if (reason instanceof ApplicationError) {
            ...
            if (reason instanceof InvalidDevtoolYMLError) {
                return new ConnectError(
                    reason.message,
                    ...
                    [
                        new FailedPreconditionDetails({
                            reason: {
                                case: "invalidDevtoolYml",
                                value: new InvalidDevtoolYMLErrorData(reason.info),
                            },
                        }),
                    ],
                    reason,
    ...
    fromError(reason: ConnectError): ApplicationError {
        ...
        if (reason.code === Code.FailedPrecondition) {
            ...
            switch (details?.reason?.case) {
                case "invalidDevtoolYml":
                    const invalidDevtoolYmlInfo = toPlainMessage(details.reason.value);
                    // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
                    return new InvalidDevtoolYMLError(invalidDevtoolYmlInfo);
                case "repositoryNotFound":
    ...
    toPrebuilds(devtoolHost: string, prebuilds: PrebuildWithStatus[]): Prebuild[] {
        return prebuilds.map((prebuild) => this.toPrebuild(devtoolHost, prebuild));
    }

    toPrebuild(devtoolHost: string, prebuild: PrebuildWithStatus): Prebuild {
        return new Prebuild({
            id: prebuild.info.id,
            ...
            status: this.toPrebuildStatus(devtoolHost, prebuild),
        });
    }

    toPrebuildStatus(devtoolHost: string, prebuild: PrebuildWithStatus): PrebuildStatus {
        const tasks: TaskLog[] = [];
        let taskIndex = 0;
                    ...
                    new TaskLog({
                        ...
                        logUrl:
                            ...
                            task.before || task.init || task.prebuild
                                ? new URL(getPrebuildLogPath(prebuild.info.id, `${i}`), devtoolHost).toString()
                                : undefined,
                    }),
                );
            }
        ...
        // This is a hack mimicking the supervisor behavior of adding dynamic IDE tasks https://github.com/khulnasoft/devtool/blob/e7d79c355e2cd6ac34056ea52d7bdcda45975839/components/ide-service/pkg/server/server.go#L508-L540
        ...
        return new PrebuildStatus({
            ...
            logUrl: new URL(getPrebuildLogPath(prebuild.info.id), devtoolHost).toString(),
            taskLogs: tasks,
            imageBuildLogUrl: new URL(getPrebuildLogPath(prebuild.workspace.id, "image-build"), devtoolHost).toString(),
        });
    }
    ...
    toOnboardingState(state: DevtoolServer.OnboardingState): OnboardingState {
    ...
    toInstallationConfiguration(config: DevtoolServerInstallationConfiguration): InstallationConfiguration {
