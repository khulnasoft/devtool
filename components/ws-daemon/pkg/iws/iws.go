...
import (
	...
	"github.com/khulnasoft/devtool/common-go/cgroups"
	v2 "github.com/khulnasoft/devtool/common-go/cgroups/v2"
	"github.com/khulnasoft/devtool/common-go/log"
	"github.com/khulnasoft/devtool/common-go/tracing"
	wsinit "github.com/khulnasoft/devtool/content-service/pkg/initializer"
	"github.com/khulnasoft/devtool/ws-daemon/api"
	"github.com/khulnasoft/devtool/ws-daemon/pkg/container"
	"github.com/khulnasoft/devtool/ws-daemon/pkg/internal/session"
	nsi "github.com/khulnasoft/devtool/ws-daemon/pkg/nsinsider"
)
...
func (wbs *InWorkspaceServiceServer) PrepareForUserNS(ctx context.Context, req *api.PrepareForUserNSRequest) (*api.PrepareForUserNSResponse, error) {
	...
	err = nsi.Nsinsider(wbs.Session.InstanceID, int(containerPID), func(c *exec.Cmd) {
		c.Args = append(c.Args, "prepare-dev", "--uid", strconv.Itoa(wsinit.DevtoolUID), "--gid", strconv.Itoa(wsinit.DevtoolGID))
	})
	if err != nil {
...
// └── workspace       drwxr-xr-x 5 devtoolUid devtoolGid
//
//	└── user        drwxr-xr-x 5 devtoolUid devtoolGid
func (wbs *InWorkspaceServiceServer) EvacuateCGroup(ctx context.Context, req *api.EvacuateCGroupRequest) (*api.EvacuateCGroupResponse, error) {
	...
	out, err := exec.CommandContext(ctx, "chown", "-R", fmt.Sprintf("%d:%d", wsinit.DevtoolUID, wsinit.DevtoolGID), filepath.Join(wbs.CGroupMountPoint, workspaceCGroup)).CombinedOutput()
	if err != nil {
