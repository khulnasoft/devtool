...
func RunInitializer(ctx context.Context, destination string, initializer *csapi.WorkspaceInitializer, remoteContent map[string]storage.DownloadInfo, opts RunInitializerOpts) (err error) {
	...
	if opts.GID == 0 {
		opts.GID = wsinit.DevtoolGID
	}
	if opts.UID == 0 {
		opts.UID = wsinit.DevtoolUID
	}
...
func RunInitializerChild() (err error) {
	...
	initializer, err := wsinit.NewFromRequest(ctx, dst, rs, &req, wsinit.NewFromRequestOpts{ForceDevtoolUserForGit: false})
	if err != nil {
		return err
	}
	...
	err = wsinit.EnsureCleanDotDevtoolDirectory(ctx, dst)
	if err != nil {
		return err
	}
