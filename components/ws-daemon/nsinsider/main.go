...
import (
	...
	_ "github.com/khulnasoft/devtool/common-go/nsenter"
...
func main() {
		...
		Commands: []*cli.Command{
			...
			},
			{
				...
				Action: func(c *cli.Context) error {
					...
					// nft add table ip devtool
					devtoolTable := nftcon.AddTable(&nftables.Table{
						Family: nftables.TableFamilyIPv4,
						Name:   "devtool",
					})

					// nft add chain ip devtool ratelimit { type filter hook postrouting priority 0 \; }
					...
					// nft add counter devtool connection_drop_stats
					nftcon.AddObject(&nftables.CounterObj{
						Table: devtoolTable,
						Name:  drop_stats,
					})

					// nft add set devtool ws-connections { type ipv4_addr; flags timeout, dynamic; }
					set := &nftables.Set{
						Table:      devtoolTable,
						Name:       "ws-connections",
					...
					// nft add rule ip devtool ratelimit ip protocol tcp ct state new meter ws-connections
	...
	err := app.Run(os.Args)
	if err != nil {
		log.WithField("instanceId", os.Getenv("DEVTOOL_INSTANCE_ID")).WithField("args", os.Args).Fatal(err)
	}
}
