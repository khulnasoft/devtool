...
func TestScrubFormatter(t *testing.T) {
	...
	createInfo := func() *TestWorkspaceInfo {
		return &TestWorkspaceInfo{
			...
			WorkspaceContextUrl: "https://github.com/khulnasoft/devtool",
		}
	}
	info := createInfo()
	workspaceID := "1234567890"
	...
	logger.WithTime(logTime).WithField("info", info).WithField("workspaceID", workspaceID).
		...
		Info("email: anton@devtool.io")

	expectation := fmt.Sprintf(
		`{"error":"some test error","info":"[redacted:nested]","level":"info","message":"email: anton@devtool.io","serviceContext":{"service":"test","version":"1.0.0"},"severity":"INFO","time":"%s","workspaceID":"[redacted:md5:e807f1fcf82d132f9bb018ca6738a19f]"}`,
...
func BenchmarkSimpleScrubFormatter(b *testing.B) {
	...
	run := func() {
		logger.WithField("workspaceID", "1234567890").Info("email: anton@devtool.io")
	}
