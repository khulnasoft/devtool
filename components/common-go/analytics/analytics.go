...
// NewFromEnvironment creates a new analytics writer based on the DEVTOOL_ANALYTICS_WRITER
...
func NewFromEnvironment() Writer {
	switch os.Getenv("DEVTOOL_ANALYTICS_WRITER") {
	case "log":
		log.Debug("log analytics")
		return &logAnalyticsWriter{}
	case "segment":
		log.Debug("segment analytics")
		client, err := segment.NewWithConfig(os.Getenv("DEVTOOL_ANALYTICS_SEGMENT_KEY"), segment.Config{
			Endpoint: os.Getenv("DEVTOOL_ANALYTICS_SEGMENT_ENDPOINT"),
		})
		if err != nil {
