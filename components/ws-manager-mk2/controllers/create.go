...
import (
	...
	wsk8s "github.com/khulnasoft/devtool/common-go/kubernetes"
	"github.com/khulnasoft/devtool/common-go/tracing"
	csapi "github.com/khulnasoft/devtool/content-service/api"
	regapi "github.com/khulnasoft/devtool/registry-facade/api"
	"github.com/khulnasoft/devtool/ws-manager-mk2/pkg/constants"
	config "github.com/khulnasoft/devtool/ws-manager/api/config"
...
const (
	...
	headlessLabel = "devtool.io/headless"
	...
	instanceIDLabel = "devtool.io/instanceID"
...
func createDefiniteWorkspacePod(sctx *startWorkspaceContext) (*corev1.Pod, error) {
	...
	labels["devtool.io/networkpolicy"] = "default"
		...
		volumes = append(volumes, corev1.Volume{
			...
			VolumeSource: corev1.VolumeSource{
				ConfigMap: &corev1.ConfigMapVolumeSource{
					LocalObjectReference: corev1.LocalObjectReference{Name: "devtool-customer-certificate-bundle"},
				},
			},
		})
	}
	...
	matchExpressions := []corev1.NodeSelectorRequirement{
		{
			Key:      "devtool.io/workload_workspace_" + workloadType,
			Operator: corev1.NodeSelectorOpExists,
		},
		{
			Key:      "devtool.io/ws-daemon_ready_ns_" + sctx.Config.Namespace,
			Operator: corev1.NodeSelectorOpExists,
		},
		{
			Key:      "devtool.io/registry-facade_ready_ns_" + sctx.Config.Namespace,
			Operator: corev1.NodeSelectorOpExists,
		},
	}
	...
	pod := corev1.Pod{
		ObjectMeta: metav1.ObjectMeta{
			...
			Finalizers:  []string{workspacev1.DevtoolFinalizerName},
		},
		Spec: corev1.PodSpec{
...
func createWorkspaceContainer(sctx *startWorkspaceContext) (*corev1.Container, error) {
	...
	if sctx.Config.EnableCustomSSLCertificate {
		volumeMounts = append(volumeMounts, corev1.VolumeMount{
			Name:      "devtool-ca-crt",
			MountPath: "/etc/ssl/certs/devtool-ca.crt",
...
func createWorkspaceEnvironment(sctx *startWorkspaceContext) ([]corev1.EnvVar, error) {
	...
	wsUrl, err := config.RenderWorkspaceURL(sctx.Config.WorkspaceURLTemplate, sctx.Workspace.Name, sctx.Workspace.Spec.Ownership.WorkspaceID, sctx.Config.DevtoolHostURL)
	if err != nil {
		return nil, fmt.Errorf("cannot render workspace URL: %w", err)
	}

	// Envs that start with DEVTOOL_ are appended to the Terminal environments
	result := []corev1.EnvVar{}
	result = append(result, corev1.EnvVar{Name: "DEVTOOL_REPO_ROOT", Value: allRepoRoots[0]})
	result = append(result, corev1.EnvVar{Name: "DEVTOOL_REPO_ROOTS", Value: strings.Join(allRepoRoots, ",")})
	result = append(result, corev1.EnvVar{Name: "DEVTOOL_OWNER_ID", Value: sctx.Workspace.Spec.Ownership.Owner})
	result = append(result, corev1.EnvVar{Name: "DEVTOOL_WORKSPACE_ID", Value: sctx.Workspace.Spec.Ownership.WorkspaceID})
	result = append(result, corev1.EnvVar{Name: "DEVTOOL_INSTANCE_ID", Value: sctx.Workspace.Name})
	result = append(result, corev1.EnvVar{Name: "DEVTOOL_THEIA_PORT", Value: strconv.Itoa(int(sctx.IDEPort))})
	...
	result = append(result, corev1.EnvVar{Name: "DEVTOOL_HOST", Value: sctx.Config.DevtoolHostURL})
	result = append(result, corev1.EnvVar{Name: "DEVTOOL_WORKSPACE_URL", Value: wsUrl})
	result = append(result, corev1.EnvVar{Name: "DEVTOOL_WORKSPACE_CLUSTER_HOST", Value: sctx.Config.WorkspaceClusterHost})
	result = append(result, corev1.EnvVar{Name: "DEVTOOL_WORKSPACE_CLASS", Value: sctx.Workspace.Spec.Class})
	...
	result = append(result, corev1.EnvVar{Name: "DEVTOOL_SSH_CA_PUBLIC_KEY", Value: sctx.Workspace.Spec.SSHGatewayCAPublicKey})

	// We don't require that Git be configured for workspaces
	if sctx.Workspace.Spec.Git != nil {
		result = append(result, corev1.EnvVar{Name: "DEVTOOL_GIT_USER_NAME", Value: sctx.Workspace.Spec.Git.Username})
		result = append(result, corev1.EnvVar{Name: "DEVTOOL_GIT_USER_EMAIL", Value: sctx.Workspace.Spec.Git.Email})
	}

	if sctx.Config.EnableCustomSSLCertificate {
		const (
			customCAMountPath = "/etc/ssl/certs/devtool-ca.crt"
	...
	for _, e := range sctx.Workspace.Spec.UserEnvVars {
		switch e.Name {
		case "DEVTOOL_WORKSPACE_CONTEXT",
			"DEVTOOL_WORKSPACE_CONTEXT_URL",
			"DEVTOOL_TASKS",
			"DEVTOOL_RESOLVED_EXTENSIONS",
			"DEVTOOL_EXTERNAL_EXTENSIONS",
			"DEVTOOL_WORKSPACE_CLASS_INFO",
			"DEVTOOL_IDE_ALIAS",
			"DEVTOOL_RLIMIT_CORE":
			// these variables are allowed - don't skip them
		default:
			if strings.HasPrefix(e.Name, "DEVTOOL_") {
				// we don't allow env vars starting with DEVTOOL_ and those that we do allow we've listed above
				continue
			}
		}
	...
	result = append(result, corev1.EnvVar{Name: "DEVTOOL_INTERVAL", Value: fmt.Sprintf("%d", int64(heartbeatInterval/time.Millisecond))})
	...
	result = append(result, corev1.EnvVar{Name: "DEVTOOL_MEMORY", Value: strconv.FormatInt(memoryInMegabyte, 10)})

	cpuCount := res.Cpu().Value()
	result = append(result, corev1.EnvVar{Name: "DEVTOOL_CPU_COUNT", Value: strconv.FormatInt(int64(cpuCount), 10)})

	if sctx.Headless {
		result = append(result, corev1.EnvVar{Name: "DEVTOOL_HEADLESS", Value: "true"})
	}
...
func createDefaultSecurityContext() (*corev1.SecurityContext, error) {
	...
	res := &corev1.SecurityContext{
		...
		RunAsGroup:             &devtoolGUID,
		RunAsNonRoot:           pointer.Bool(true),
		RunAsUser:              &devtoolGUID,
	}
