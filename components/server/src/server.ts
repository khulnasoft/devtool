...
import { createWebSocketConnection, toIWebSocket } from "@khulnasoft/devtool-protocol/lib/messaging/node/connection";
...
import { increaseHttpRequestCounter, observeHttpRequestDuration, setDevtoolVersion } from "./prometheus-metrics";
...
export class Server {
    ...
    public async init(app: express.Application) {
        ...
        setDevtoolVersion(this.config.version);
        ...
        this.eventEmitter.on(Server.EVENT_ON_START, (httpServer) => {
            ...
            //  - for cookie/session AND Bearer auth: MUST be hostUrl.hostname (devtool.io)
            //  - edge case: empty "Origin" is always permitted
            // We rely on the origin header being set correctly (needed by regular clients to use Devtool:
            // CORS allows subdomains to access devtool.io)
