...
export class BitbucketService extends RepositoryService {
    ...
    public async isDevtoolWebhookEnabled(user: User, cloneUrl: string): Promise<boolean> {
        try {
            ...
            console.error("Failed to check if Devtool webhook is enabled.", error, { cloneUrl });
