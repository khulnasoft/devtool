...
/**
 * GitHub app urls:
 *    User authorization callback URL: https://devtool.io/install-github-app
 *    Setup URL:                       https://devtool.io/install-github-app
 *    Webhook URL:                     https://devtool.io/apps/github
 *
 * Make sure that the webhook secret you set in GitHub matches what's in your
 * values.yaml file (DEVTOOL_GITHUB_APP_WEBHOOK_SECRET) - it's not a bad idea to
...
export class GithubApp {
    ...
    private async buildApp(app: Probot, options: ApplicationFunctionOptions) {
        ...
        // Backward-compatibility: Redirect old badge URLs (e.g. "/api/apps/github/pbs/github.com/khulnasoft/devtool/5431d5735c32ab7d5d840a4d1a7d7c688d1f0ce9.svg")
        options.getRouter &&
            options
                .getRouter("/pbs")
    ...
    private async handlePushEvent(ctx: Context<"push">): Promise<void> {
        ...
        // https://github.com/khulnasoft/devtool/issues/11578
    ...
    private getBadgeImageURL(): string {
        return this.config.hostUrl.with({ pathname: "/button/open-in-devtool.svg" }).toString();
    }
