...
const shouldSkip =
    ifEnvVarNotSet("DEVTOOL_TEST_TOKEN_BITBUCKET_SERVER_READ") &&
    ifEnvVarNotSet("DEVTOOL_TEST_TOKEN_BITBUCKET_SERVER_WRITE") &&
    ifEnvVarNotSet("DEVTOOL_TEST_TOKEN_BITBUCKET_SERVER_ADMIN");
...
class TestBitbucketServerTokenValidator {
    ...
    // https://bitbucket.devtool-dev.com/projects/TES/repos/read-write-permission/permissions
    ...
    @test(skip(ifEnvVarNotSet("DEVTOOL_TEST_TOKEN_BITBUCKET_SERVER_READ"))) async test_checkWriteAccess_read_only() {
        const token = process.env["DEVTOOL_TEST_TOKEN_BITBUCKET_SERVER_READ"]!;
    ...
    @test(skip(ifEnvVarNotSet("DEVTOOL_TEST_TOKEN_BITBUCKET_SERVER_WRITE")))
    async test_checkWriteAccess_write_permissions() {
        const token = process.env["DEVTOOL_TEST_TOKEN_BITBUCKET_SERVER_WRITE"]!;
        const result = await this.getValidator(token).checkWriteAccess(Object.assign({}, this.checkParams, { token }));
        expect(result).to.deep.equal({
            found: true,
            isPrivateRepo: true,
    ...
    @test(skip(ifEnvVarNotSet("DEVTOOL_TEST_TOKEN_BITBUCKET_SERVER_ADMIN")))
    async test_checkWriteAccess_admin_permissions() {
        const token = process.env["DEVTOOL_TEST_TOKEN_BITBUCKET_SERVER_ADMIN"]!;
        const result = await this.getValidator(token).checkWriteAccess(Object.assign({}, this.checkParams, { token }));
        expect(result).to.deep.equal({
            found: true,
            isPrivateRepo: true,
