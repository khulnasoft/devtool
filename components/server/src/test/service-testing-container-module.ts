...
export const mockAuthConfig: AuthConfig = {
    ...
    session: {
        issuer: "https://mp-server-d7650ec945.preview.devtool-dev.com",
        lifetimeSeconds: 7 * 24 * 60 * 60,
        cookie: {
            name: "__Host-_devtool_dev_jwt_",
            secure: true,
            httpOnly: true,
            maxAge: 7 * 24 * 60 * 60,
            sameSite: "lax",
...
const mockApplyingContainerModule = new ContainerModule((bind, unbound, isbound, rebind) => {
                    ...
                    <IImageBuilderClient & IWorkspaceManagerClient>{
                        ...
                        build(request, metadata, options) {
                            ...
                            nextTick(() => {
                                ...
                                logInfo.setUrl("https://ws.devtool.io/my-test-image-build/logs");
                        ...
                        startWorkspace(request, metadata, options, callback) {
                            ...
                            response.setUrl(`https://${workspaceId}.ws.devtool.io`);
                            callback(null, response);
                        },
                    },
                ) as any;
    ...
    rebind(IDEServiceDefinition.name).toConstantValue(
        createMock(<Partial<IDEServiceClient>>{
            async resolveWorkspaceConfig() {
                return {
                    envvars: [],
                    supervisorImage: "devtool/supervisor:latest",
                    webImage: "devtool/code:latest",
                    ideImageLayers: [],
                    refererIde: "code",
                    ideSettings: "",
                    tasks: "",
    ...
    rebind<Partial<Config>>(Config).toConstantValue({
        ...
        workspaceDefaults: {
            workspaceImage: "devtool/workspace-full",
            defaultFeatureFlags: [],
            previewFeatureFlags: [],
        },
        workspaceClasses: [
