...
export class EnvVarService {
    ...
    validateProjectEnvVar(envVar: Partial<ProjectEnvVarWithValue>) {
        ...
        if (!UserEnvVar.WhiteListFromReserved.includes(envVar.name) && envVar.name.startsWith("DEVTOOL_")) {
            throw new ApplicationError(ErrorCodes.BAD_REQUEST, "Variable name with prefix 'DEVTOOL_' is reserved");
        }
    ...
    async resolveEnvVariables(
        ...
        // 1. first merge the `env` in the .devtool.yml
        if (wsConfig?.env) {
