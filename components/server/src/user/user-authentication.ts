...
import { EmailDomainFilterDB, MaybeUser, UserDB } from "@khulnasoft/devtool-db/lib";
...
export class UserAuthentication {
    ...
    async deauthorize(user: User, authProviderId: string) {
        const externalIdentities = user.identities.filter(
            (i) => i.authProviderId !== TokenService.DEVTOOL_AUTH_PROVIDER_ID,
        );
        ...
        if (remainingLoginIdentities.length === 0) {
            throw new Error(
                "Cannot remove last authentication provider for logging in to Devtool. Please delete account if you want to leave.",
            );
        }
    ...
    /**
     * devtool.io: Only installation-level users are allowed to create orgs
