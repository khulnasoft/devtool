...
export class TokenService implements TokenProvider {
    static readonly DEVTOOL_AUTH_PROVIDER_ID = "Devtool";
    /**
     * [mins]
     *
    ...
    async getOrCreateDevtoolIdentity(user: User): Promise<Identity> {
        let identity = User.getIdentity(user, TokenService.DEVTOOL_AUTH_PROVIDER_ID);
        if (!identity) {
            identity = {
                authProviderId: TokenService.DEVTOOL_AUTH_PROVIDER_ID,
                authId: user.id,
    ...
    async createDevtoolToken(user: User, ...scopes: string[]): Promise<TokenEntry> {
        const identity = await this.getOrCreateDevtoolIdentity(user);
