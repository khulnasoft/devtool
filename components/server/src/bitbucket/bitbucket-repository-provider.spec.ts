...
@suite(timeout(10000), retries(0), skip(ifEnvVarNotSet("DEVTOOL_TEST_TOKEN_BITBUCKET")))
class TestBitbucketRepositoryProvider {
    ...
    @test public async testGetRepo() {
        const result = await this.repoProvider.getRepo(this.user, "devtool", "integration-tests");
        expect(result).to.deep.include({
            ...
            webUrl: "https://bitbucket.org/devtool/integration-tests",
        });
    }
    ...
    @test public async testHasReadAccess_positive() {
        const result = await this.repoProvider.hasReadAccess(this.user, "devtool", "integration-tests");
    ...
    @test public async testSearchRepos_matchesSubstring() {
        ...
        expect(result).to.deep.include({
            url: "https://bitbucket.org/devtool-integration-tests/exampul-repo",
            name: "exampul-repo",
        });
    }
    ...
    @test public async testSearchRepos_doesNotMatchPath() {
        const result = await this.repoProvider.searchRepos(this.user, "devtool-integration-tests/exampul-repo", 100);
