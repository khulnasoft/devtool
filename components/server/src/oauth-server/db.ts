...
const localClient: OAuthClient = {
    ...
    name: "Devtool local control client",
    ...
    scopes: [
        { name: "function:getDevtoolTokenScopes" },
...
const localCliClientID = "devtool-cli";
...
const jetBrainsGateway: OAuthClient = {
    id: "jetbrains-gateway-devtool-plugin",
    name: "JetBrains Gateway Devtool Plugin",
    // Set of valid redirect URIs
    ...
    redirectUris: Array.from(
        { length: 20 },
        (_, i) => `http://127.0.0.1:${63342 + i}/api/devtool/oauth/authorization_code`,
    ),
    allowedGrants: ["authorization_code"],
    scopes: [
        { name: "function:getDevtoolTokenScopes" },
...
function createVSCodeClient(protocol: string, displayName: string): OAuthClient {
    return {
        id: `${protocol}-devtool`,
        name: `${displayName}: Devtool extension`,
        redirectUris: [protocol + "://devtool.devtool-desktop/complete-devtool-auth"],
        allowedGrants: ["authorization_code"],
        scopes: [
            { name: "function:getDevtoolTokenScopes" },
            { name: "function:getLoggedInUser" },
...
const desktopClient: OAuthClient = {
    id: "devtool-desktop",
    name: "Devtool Desktop",
    redirectUris: ["devtool://complete-auth"],
    allowedGrants: ["authorization_code"],
    scopes: [
        { name: "function:getDevtoolTokenScopes" },
        { name: "function:getLoggedInUser" },
...
const toolbox: OAuthClient = {
    id: "toolbox-gateway-devtool-plugin",
    name: "JetBrains Toolbox Devtool Plugin",
    redirectUris: ["jetbrains://gateway/io.devtool.toolbox.gateway/auth"],
    allowedGrants: ["authorization_code"],
    scopes: [
        { name: "function:getDevtoolTokenScopes" },
        { name: "function:getLoggedInUser" },
