...
export interface StartWorkspaceOptions extends Omit<DevtoolServer.StartWorkspaceOptions, "ideSettings"> {
...
export class WorkspaceStarter {
    ...
    public async stopWorkspaceInstance(
        ...
        try {
            ...
            if (workspace && instance && instance?.status.phase !== "stopped") {
                ...
                await this.userDB.trace({ span }).deleteDevtoolTokensNamedLike(workspace.ownerId, `${instance.id}-%`);
        ...
        if (blockedRepository.blockFreeUsage) {
            ...
            if (tier === "free") {
                throw new ApplicationError(
                    ...
                    `${contextURL} requires a paid plan on Devtool.`,
                );
            }
        }
        if (!blockedRepository.blockFreeUsage) {
            throw new ApplicationError(ErrorCodes.PRECONDITION_FAILED, `${contextURL} is blocklisted on Devtool.`);
        }
    }
    ...
    private async getAdditionalImageAuth(envVars: ResolvedEnvVars): Promise<Map<string, string>> {
        ...
        const imageAuth = envVars.project.find((e) => e.name === "DEVTOOL_IMAGE_AUTH");
        if (!imageAuth) {
            return res;
        }
    ...
    private async createSpec(
        ...
        contextUrlEnv.setName("DEVTOOL_WORKSPACE_CONTEXT_URL");
        ...
        contextEnv.setName("DEVTOOL_WORKSPACE_CONTEXT");
        ...
        if (!!info) {
            ...
            workspaceClassInfoEnv.setName("DEVTOOL_WORKSPACE_CLASS_INFO");
        ...
        const tasks = resolveDevtoolTasks(workspace, instance);
        if (tasks.length) {
        ...
        if (workspace.config.coreDump?.enabled) {
            ...
            rLimitCore.setName("DEVTOOL_RLIMIT_CORE");
            rLimitCore.setValue(
                JSON.stringify({
        ...
        const createDevtoolTokenPromise = (async () => {
            const scopes = this.createDefaultDevtoolAPITokenScopes(workspace, instance);
            const token = crypto.randomBytes(30).toString("hex");
            const tokenHash = crypto.createHash("sha256").update(token, "utf8").digest("hex");
            const dbToken: DevtoolToken = {
                tokenHash,
                name: `${instance.id}-default`,
                type: DevtoolTokenType.MACHINE_AUTH_TOKEN,
                userId: user.id,
                scopes,
                created: new Date().toISOString(),
            };
            await this.userDB.trace(traceCtx).storeDevtoolToken(dbToken);
        ...
        orgIdEnv.setName("DEVTOOL_DEFAULT_WORKSPACE_IMAGE");
        ...
        const [isSetJavaXmx, isSetJavaProcessorCount, disableJetBrainsLocalPortForwarding] = await Promise.all([
            client
                ...
                .then((v) => newEnvVar("DEVTOOL_IS_SET_JAVA_XMX", String(v))),
            client
                .getValueAsync("supervisor_set_java_processor_count", false, { user })
                .then((v) => newEnvVar("DEVTOOL_IS_SET_JAVA_PROCESSOR_COUNT", String(v))),
            client
                .getValueAsync("disable_jetbrains_local_port_forwarding", false, { user })
                .then((v) => newEnvVar("DEVTOOL_DISABLE_JETBRAINS_LOCAL_PORT_FORWARDING", String(v))),
        ]);
        ...
        await createDevtoolTokenPromise;
    ...
    private createDefaultDevtoolAPITokenScopes(workspace: Workspace, instance: WorkspaceInstance): string[] {
        const scopes = [
            ...
            "function:openPort",
            "function:closePort",
            "function:generateNewDevtoolToken",
            ...
            // getIDToken is used by Devtool's OIDC Identity Provider to check for authorisation.
...
function resolveDevtoolTasks(ws: Workspace, instance: WorkspaceInstance): TaskConfig[] {
