...
export class ConfigProvider {
    @inject(DevtoolFileParser) protected readonly devtoolParser: DevtoolFileParser;
    ...
    public async fetchCustomConfig(
        ...
        try {
            ...
            if (AdditionalContentContext.is(commit) && commit.additionalFiles[".devtool.yml"]) {
                customConfigString = commit.additionalFiles[".devtool.yml"];
                const parseResult = this.devtoolParser.parse(customConfigString);
                customConfig = parseResult.config;
                customConfig._origin = "additional-content";
                if (parseResult.validationErrors) {
                    const err = new InvalidDevtoolYMLError({
            ...
            if (!customConfig) {
                ...
                const contextRepoConfig = services.fileProvider.getDevtoolFileContent(commit, user);
            ...
            return { customConfig, configBasePath, literalConfig: { ".devtool.yml": customConfigString || "" } };
        } catch (e) {
            TraceContext.setError({ span }, e);
            throw e;
        } finally {
    ...
    private async validateConfig(config: WorkspaceConfig, user: User): Promise<void> {
        ...
        if (checkoutLocation) {
            ...
            if (this.leavesWorkspaceBase(normalizedPath)) {
                ...
                throw new Error(
                    `Checkout location must not leave the ${POD_PATH_WORKSPACE_BASE} folder. Check your .devtool.yml file.`,
                );
            }
        }
        ...
        if (workspaceLocation) {
            ...
            if (this.leavesWorkspaceBase(normalizedPath)) {
                ...
                throw new Error(
                    `Workspace location must not leave the ${POD_PATH_WORKSPACE_BASE} folder. Check your .devtool.yml file.`,
                );
            }
        }
    }
