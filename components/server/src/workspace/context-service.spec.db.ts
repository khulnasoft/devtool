...
const SNAPSHOT_BUCKET = "https://devtool.io/none-bucket";
...
describe("ContextService", async () => {
    ...
    beforeEach(async () => {
            ...
            container.rebind(ContextParser).toConstantValue({
                ...
                handle: async function (ctx: TraceContext, user: User, contextURL: string): Promise<WorkspaceContext> {
                    ...
                    cases.set("https://github.com/khulnasoft/empty", () => {
                        return devtoolEmptyContext as any;
                    });
                    ...
                    return createCommitContextForBranch(devtoolEmptyContext.repository.defaultBranch);
                },
        ...
        mockRepositoryProvider.addBranch({ name: "main", htmlUrl: "https://github.com/khulnasoft/empty/tree/main" }, [
            {
                sha: devtoolEmptyContext.revision,
                author: "some-dude",
    ...
    it("should parse normal context", async () => {
        ...
        expect(ctx.context.ref).to.equal(devtoolEmptyContext.ref);
        expect((ctx.context as CommitContext).revision).to.equal(devtoolEmptyContext.revision);
    });
...
async function createTestWorkspace(svc: WorkspaceService, org: Organization, owner: User, project: Project) {
    const ws = await svc.createWorkspace(
        ...
        org.id,
        project,
        devtoolEmptyContext as any as CommitContext,
