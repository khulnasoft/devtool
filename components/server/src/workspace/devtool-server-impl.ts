...
import {
    ...
    DevtoolClient as DevtoolApiClient,
    DevtoolServer,
    DevtoolToken,
    DevtoolTokenType,
    PermissionName,
...
import { DevtoolTokenService } from "../user/devtool-token-service";
...
export type DevtoolServerWithTracing = InterfaceWithTraceContext<DevtoolServer>;

@injectable()
export class DevtoolServerImpl implements DevtoolServerWithTracing, Disposable {
    constructor(
        ...
        @inject(DevtoolTokenService) private readonly devtoolTokenService: DevtoolTokenService,
    ...
    private client: DevtoolApiClient | undefined;
    ...
    initialize(
        client: DevtoolApiClient | undefined,
    ...
    private async listenForPrebuildUpdates() {
        ...
        const disableWebsocketPrebuildUpdates = await getExperimentsClientForBackend().getValueAsync(
            ...
            false,
            {
                devtoolHost: this.config.hostUrl.url.host,
            },
        );
    ...
    private async getAccessibleProjects() {
        ...
        return runWithRequestContext(
            {
                requestKind: "devtool-server-impl-listener",
    ...
    setClient(ctx: TraceContext, client: DevtoolApiClient | undefined): void {
    ...
    public async getToken(ctx: TraceContext, query: DevtoolServer.GetTokenSearchOptions): Promise<Token | undefined> {
    ...
    public async startWorkspace(
        ctx: TraceContext,
        workspaceId: string,
        options: DevtoolServer.StartWorkspaceOptions,
    ...
    public async getWorkspaces(
        ctx: TraceContext,
        options: DevtoolServer.GetWorkspacesOptions,
    ...
    public async sendHeartBeat(ctx: TraceContext, options: DevtoolServer.SendHeartBeatOptions): Promise<void> {
        traceAPIParams(ctx, { options });
    ...
    public async createWorkspace(
        ctx: TraceContext,
        options: DevtoolServer.CreateWorkspaceOptions,
    ...
    async takeSnapshot(ctx: TraceContext, options: DevtoolServer.TakeSnapshotOptions): Promise<string> {
        traceAPIParams(ctx, { options });
    ...
    public async getDevtoolTokens(ctx: TraceContext): Promise<DevtoolToken[]> {
        const user = await this.checkAndBlockUser("getDevtoolTokens");
        const devtoolTokens = await this.devtoolTokenService.getDevtoolTokens(user.id, user.id);
        await Promise.all(devtoolTokens.map((tkn) => this.guardAccess({ kind: "devtoolToken", subject: tkn }, "get")));
        return devtoolTokens;
    }

    public async generateNewDevtoolToken(
        ctx: TraceContext,
    ...
    public async getDevtoolTokenScopes(ctx: TraceContext, tokenHash: string): Promise<string[]> {
        ...
        const user = await this.checkAndBlockUser("getDevtoolTokenScopes");
        const devtoolToken = await this.devtoolTokenService.findDevtoolToken(user.id, user.id, tokenHash);
        if (devtoolToken) {
            await this.guardAccess({ kind: "devtoolToken", subject: devtoolToken }, "get");
        }
        return devtoolToken?.scopes ?? [];
    }

    public async deleteDevtoolToken(ctx: TraceContext, tokenHash: string): Promise<void> {
        traceAPIParams(ctx, {}); // do not trace tokenHash

        const user = await this.checkAndBlockUser("deleteDevtoolToken");
        return this.devtoolTokenService.deleteDevtoolToken(user.id, user.id, tokenHash, (token: DevtoolToken) => {
            return this.guardAccess({ kind: "devtoolToken", subject: token }, "delete");
        });
    }
    ...
    async updateOwnAuthProvider(
        ctx: TraceContext,
        { entry }: DevtoolServer.UpdateOwnAuthProviderParams,
    ...
    private redactUpdateOwnAuthProviderParams({ entry }: DevtoolServer.UpdateOwnAuthProviderParams) {
        const safeEntry =
            "id" in entry
    ...
    async deleteOwnAuthProvider(ctx: TraceContext, params: DevtoolServer.DeleteOwnAuthProviderParams): Promise<void> {
        traceAPIParams(ctx, { params });
    ...
    async createOrgAuthProvider(
        ctx: TraceContext,
        { entry }: DevtoolServer.CreateOrgAuthProviderParams,
    ): Promise<AuthProviderEntry> {
        traceAPIParams(ctx, {}); // entry contains PII
    ...
    async updateOrgAuthProvider(
        ctx: TraceContext,
        { entry }: DevtoolServer.UpdateOrgAuthProviderParams,
    ): Promise<AuthProviderEntry> {
        traceAPIParams(ctx, {}); // entry contains PII
    ...
    async getOrgAuthProviders(
        ctx: TraceContext,
        params: DevtoolServer.GetOrgAuthProviderParams,
    ): Promise<AuthProviderEntry[]> {
        traceAPIParams(ctx, { params });
    ...
    async deleteOrgAuthProvider(ctx: TraceContext, params: DevtoolServer.DeleteOrgAuthProviderParams): Promise<void> {
        traceAPIParams(ctx, { params });
    ...
    async getOnboardingState(ctx: TraceContext): Promise<DevtoolServer.OnboardingState> {
