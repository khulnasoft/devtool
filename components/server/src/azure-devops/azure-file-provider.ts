...
export class AzureDevOpsFileProvider implements FileProvider {
    ...
    public async getDevtoolFileContent(commit: Commit, user: User): Promise<MaybeContent> {
        ...
        const yamlVersion1 = await Promise.all([
            this.azureDevOpsApi.getFileContent(user, azOrgId, azProject, repoName, commit, ".devtool.yml"),
            this.azureDevOpsApi.getFileContent(user, azOrgId, azProject, repoName, commit, ".devtool"),
        ]);
