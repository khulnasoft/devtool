...
export abstract class GenericAuthProvider implements AuthProvider {
    ...
    async callback(request: express.Request, response: express.Response, next: express.NextFunction): Promise<void> {
        ...
        if (flowContext) {
            ...
            if (VerifyResult.WithIdentity.is(flowContext)) {
                ...
                // Set all cookies used on website for visitor preferences for .devtool.io domain if no preference exists yet
    ...
    protected async verify(
        ...
        const authProviderId = this.authProviderId;
        let currentDevtoolUser: User | undefined = User.is(req.user) ? req.user : undefined;
        ...
        try {
            ...
            if (currentDevtoolUser) {
                // user is already logged in

                // check for matching auth ID
                const currentIdentity = currentDevtoolUser.identities.find(
                ...
                if (currentIdentity && currentIdentity.authId !== candidate.authId) {
                    log.warn(`User is trying to connect with another provider identity.`, {
                        ...
                        authUser,
                        candidate,
                        currentDevtoolUser: currentDevtoolUser.id,
                        clientInfo,
                    });
                    done(
                ...
                currentDevtoolUser = await this.userAuthentication.findUserForLogin({ candidate });
            ...
            if (currentDevtoolUser) {
                const elevateScopes = authFlow.overrideScopes
                    ? undefined
                    : await this.getMissingScopeForElevation(currentDevtoolUser, currentScopes);
                const isBlocked = await this.userAuthentication.isBlocked({ user: currentDevtoolUser });
            ...
            done(undefined, currentDevtoolUser || candidate, flowContext);
        } catch (err) {
...
interface GenericOAuthStrategyOptions {
    scope?: string | string[];
    /**
     * This should be Devtool's hostname.
     */
    userAgent: string;
