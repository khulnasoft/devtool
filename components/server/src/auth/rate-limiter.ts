...
type DevtoolServerMethodType =
    | keyof Omit<DevtoolServer, "dispose" | "setClient">
 ...
 *
 * @param name
 * @returns True iff the name is a valid function of the DevtoolServer interface. name is case sensitive.
 */
export function isValidFunctionName(name: string): boolean {
    const valid: boolean = !!defaultFunctions[name as any as DevtoolServerMethodType];
    return valid;
}
...
const defaultFunctions: FunctionsConfig = {
    ...
    getDevtoolTokenScopes: { group: "default", points: 1 },
    ...
    getDevtoolTokens: { group: "default", points: 1 },
    generateNewDevtoolToken: { group: "default", points: 1 },
    deleteDevtoolToken: { group: "default", points: 1 },
...
export class UserRateLimiter {
    ...
    async consume(user: string, method: string): Promise<RateLimiterRes> {
        const group = this.config.functions[method as DevtoolServerMethodType]?.group || "default";
        if (group !== this.config.functions[method as DevtoolServerMethodType]?.group) {
        ...
        const points = this.config.functions[method as DevtoolServerMethodType]?.points || 1;
