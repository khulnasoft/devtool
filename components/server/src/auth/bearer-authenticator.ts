...
export class BearerAuth {
        ...
        return async (req, res, next) => {
            try {
                ...
                if (isBearerAuthError(e)) {
                    // (AT) while investigating https://github.com/khulnasoft/devtool/issues/8703 we
    ...
    private async userAndScopesFromToken(token: string): Promise<{ user: User; scopes: string[] }> {
        ...
        //  1. Personal Access Tokens which are prefixed with `devtool_pat_`
        ...
        const userAndToken = await this.userDB.findUserByDevtoolToken(hash, DevtoolTokenType.API_AUTH_TOKEN);
        if (!userAndToken) {
...
export class PersonalAccessToken {
    ...
    private static PAT_PREFIX = "devtool_pat_";
