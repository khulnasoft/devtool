...
func runWithinNetns() (err error) {
	...
	if opts.RuncFacade {
		args = append(args,
			"--add-runtime", "devtool="+filepath.Join(opts.BinDir, "runc-facade"),
			"--default-runtime", "devtool",
		)
	}
	...
	// cmp. ENT-324: Required to run dockerd >= 26.1 in a Devtool workspace
...
func convertRemapUser(arg, value string) ([]string, error) {
	...
	return []string{"--userns-remap", "devtool"}, nil
}
...
func adaptSubid(oldfile string, id int) error {
	...
	if id != 0 {
		newfile.WriteString(mappingFmt("devtool", 1, id))
		newfile.WriteString(mappingFmt("devtool", devtoolUserId, 1))
	} else {
		newfile.WriteString(mappingFmt("devtool", devtoolUserId, 1))
		newfile.WriteString(mappingFmt("devtool", 1, devtoolUserId-1))
		newfile.WriteString(mappingFmt("devtool", devtoolUserId+1, 32200)) // map rest of user ids in the user namespace
	}

	uidScanner := bufio.NewScanner(uid)
	for uidScanner.Scan() {
		l := uidScanner.Text()
		if !strings.HasPrefix(l, "devtool") {
