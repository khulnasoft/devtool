...
var loginCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if loginOpts.OrganizationID == "" {
			clnt, err := getDevtoolClient(config.ToContext(context.Background(), cfg))
			if err != nil {
				return fmt.Errorf("cannot connect to Devtool with this context: %w", err)
			}
			if !loginOpts.NonInteractive {
		...
		client, err := getDevtoolClient(config.ToContext(cmd.Context(), cfg))
		if err != nil {
			return err
		}
...
func init() {
	...
	if v := os.Getenv("DEVTOOL_HOST"); v != "" {
		host = v
	}
	loginCmd.Flags().StringVar(&loginOpts.Host, "host", host, "The Devtool instance to log in to (defaults to $DEVTOOL_HOST)")
	loginCmd.Flags().StringVar(&loginOpts.Token, "token", "", "The token to use for authentication (defaults to $DEVTOOL_TOKEN)")
