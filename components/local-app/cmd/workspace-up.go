...
var workspaceUpCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if workspaceCreateOpts.WorkspaceClass != "" {
			resp, err := devtool.Workspaces.ListWorkspaceClasses(cmd.Context(), connect.NewRequest(&v1.ListWorkspaceClassesRequest{}))
			if err != nil {
		...
		if workspaceCreateOpts.Editor != "" {
			resp, err := devtool.Editors.ListEditorOptions(cmd.Context(), connect.NewRequest(&v1.ListEditorOptionsRequest{}))
			if err != nil {
		...
		for {
			...
			if parentDir == currentDir {
				...
				return prettyprint.AddResolution(fmt.Errorf("no Git repository found"),
					...
					"open a remote repository using `{devtool} workspace create <repo-url>`",
				)
			}
		...
		newWorkspace, err := devtool.Workspaces.CreateAndStartWorkspace(ctx, connect.NewRequest(
			&v1.CreateAndStartWorkspaceRequest{
				Source:         &v1.CreateAndStartWorkspaceRequest_ContextUrl{ContextUrl: "DEVTOOLCLI_CONTENT_INIT=push/https://github.com/khulnasoft/empty"},
		...
		ws, err := helper.ObserveWorkspaceUntilStarted(ctx, devtool, workspaceID)
		if err != nil {
			return err
		}
		slog.Debug("workspace started", "workspaceID", workspaceID)

		token, err := devtool.Workspaces.GetOwnerToken(ctx, connect.NewRequest(&v1.GetOwnerTokenRequest{WorkspaceId: workspaceID}))
		if err != nil {
			return err
		}
		var (
		...
		pushcmd := exec.Command("git", "push", "--progress", "devtool")
		...
		case workspaceCreateOpts.StartOpts.OpenEditor:
			return helper.OpenWorkspaceInPreferredEditor(ctx, devtool, workspaceID)
		default:
