	...
	RunCommandTests(t, []CommandTest{
		...
		},
		{
			...
			PrepServer: func(mux *http.ServeMux) {
				mux.Handle(devtool_experimental_v1connect.NewWorkspacesServiceHandler(&testWorkspaceListCmdWorkspaceSrv{
...
type testWorkspaceListCmdWorkspaceSrv struct {
	Resp *v1.ListWorkspacesResponse
	Err  error
	devtool_experimental_v1connect.UnimplementedWorkspacesServiceHandler
}
