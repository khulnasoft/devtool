...
var rootCmd = &cobra.Command{
	Use:   "devtool",
	Short: "Devtool: Always ready to code.",
	Long: color.Sprint(`
<fg=ff971d>      .-+*#+           </>      <b>Devtool: Always ready to code.</>
<fg=ff971d>   :=*#####*.          </>      Try the following commands to get started:
<fg=ff971d>  .=*####*+-.    .--:  </>
<fg=ff971d>  +****=:     :=*####+ </>      devtool login              <lightgray>Login to Devtool</>
<fg=ff971d>  ****:   .-+*########.</>      devtool whoami             <lightgray>Show information about the currently logged in user</>
<fg=ff971d>  +***:   *****+--####.</>
<fg=ff971d>  +***:   .-=:.  .#*##.</>      devtool workspace list     <lightgray>List your workspaces</>
<fg=ff971d>  +***+-.      .-+**** </>      devtool workspace create   <lightgray>Create a new workspace</>
<fg=ff971d>  .=*****+=::-+*****+: </>      devtool workspace open     <lightgray>Open a running workspace</>
<fg=ff971d>  .:=+*********=-.     </>      devtool workspace stop     <lightgray>Stop a running workspace</>
<fg=ff971d>      .-++++=:         </>
...
func init() {
	...
	if fn := os.Getenv("DEVTOOL_CONFIG"); fn != "" {
		configLocation = fn
	}
...
var rootTestingOpts struct {
	Client    *client.Devtool
	WriterOut io.Writer
}

var clientCache *client.Devtool

func getDevtoolClient(ctx context.Context) (*client.Devtool, error) {
	...
	if host == nil {
		return nil, prettyprint.AddResolution(fmt.Errorf("active context has no host configured"),
			"set a host using `devtool config set-context --current --host <host>`",
			"login again using `devtool login`",
			"change to a different context using `devtool config use-context <context>`",
		)
	}
	...
	if token == "" {
		return nil, prettyprint.AddResolution(fmt.Errorf("no token found for active context"),
			"provide a token by setting the DEVTOOL_TOKEN environment variable",
			"login again using `devtool login`",
			"change to a different context using `devtool config use-context <context>`",
			"set a token explicitly using `devtool config set-context --current --token <token>`",
		)
	}

	var apiHost = *gpctx.Host.URL
	apiHost.Host = "api." + apiHost.Host
	slog.Debug("establishing connection to Devtool", "host", apiHost.String())
	res, err := client.New(
