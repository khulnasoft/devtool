...
var configContextModifyCmd = &cobra.Command{
	...
	Short:   "Modifies a context entry in the devtool CLI config",
		...
		if configContextModifyOpts.Current {
			if len(args) > 0 {
				return prettyprint.AddResolution(fmt.Errorf("cannot use --current and specify a context name"),
					"modify current context with `{devtool} config set-context --current`",
					"modify/create a different context with `{devtool} config set-context <name>`",
				)
			}
