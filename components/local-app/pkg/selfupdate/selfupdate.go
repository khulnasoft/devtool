...
const (
	// DevtoolCLIBasePath is the path relative to a Devtool installation where the latest
	// binary and manifest can be found.
	DevtoolCLIBasePath = "/static/bin"
)
...
func DownloadManifest(ctx context.Context, baseURL string) (res *Manifest, err error) {
	defer func() {
		if err != nil {
			pth := strings.TrimSuffix(baseURL, "/") + DevtoolCLIBasePath
	...
	murl.Path = filepath.Join(murl.Path, DevtoolCLIBasePath)
...
func ReplaceSelf(ctx context.Context, manifest *Manifest) error {
	...
	if err != nil && strings.Contains(err.Error(), "permission denied") && runtime.GOOS != "windows" {
		...
		err = prettyprint.AddResolution(err,
			fmt.Sprintf("run `sudo {devtool} --config %s version update`", cfgfn),
		)
	}
	return err
}
