...
## How to develop in devtool.io

### Against any* Devtool installation

Devtool installations have a feature that - if you are authorized - allow different versions of the dashboard. This allows for front-end development with live data and super-quick turnarounds.

**Preconditions**
 1. logged in user on the respective Devtool installation (e.g. devtool.example.org)
 1. user has the `"developer"` role

**Steps**
 1. Start a workspace (on any installation), and start the dev-server with `yarn start-local`
 1. Configure your browser to always send header `X-Frontend-Dev-URL` with value set to the result of `gp url 3000` to the Devtool installation you want to modify (devtool.example.org)
 1. Visit https://devtool.example.org, start modifying your `dashboard` in your workspace, and experience the effect live (incl. hot reloading)
...
Set the following 2 [environment variables](https://www.devtool.io/docs/environment-variables) either [via your account settings](https://devtool.io/variables) or [via the command line](https://www.devtool.io/docs/environment-variables#using-the-command-line-gp-env).
...
You can always go to your account settings and edit the scope for each variable to something like `*/devtool`.

```bash
# Use "devtool.io" for the SaaS version of Devtool, or specify the host of your self-hosted devtool
GP_DEV_HOST=devtool.io

# Notice the cookie name (_devtool_io_v2_) may be different if self-hosted.
# Read below for how to get the actual value to use instead of "AUTHENTICATION_COOKIE_VALUE"
GP_DEV_COOKIE="_devtool_io_v2_=AUTHENTICATION_COOKIE_VALUE"
```

Replace `AUTHENTICATION_COOKIE_VALUE` with the value of your auth cookie taken from your browser's dev tools while visiting your target Devtool host (e.g. `s%3Axxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx.XXXXXXXXXXXXXXX`).
...
You can view the dashboard at https://`PORT_NUMBER`-`DEVTOOL_WORKSPACE_URL` (`PORT_NUMBER` is usually `3000`).
