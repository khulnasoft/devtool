...
export const SetupCompleteStep: FC<Props> = ({ onComplete }) => {
        ...
        copyToClipboard(url)
            ...
            .catch((error) => {
                if (error instanceof DOMException) {
                    setCopyError(
                        "Devtool is not allowed to copy to clipboard. Please copy the URL manually or adjust your browser permissions.",
                    );
                    return;
                }
    ...
    return (
        <SetupLayout showOrg noMaxWidth>
            <Heading1>Welcome to Devtool</Heading1>
            <Subheading>Your teammates can now sign in to Devtool using single sign-on (SSO).</Subheading>
