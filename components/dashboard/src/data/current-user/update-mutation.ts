...
import { AdditionalUserData, User as UserProtocol } from "@khulnasoft/devtool-protocol";
...
export const useUpdateCurrentUserMutation = () => {
    return useMutation({
        mutationFn: async (partialUser: UpdateCurrentUserArgs) => {
            const current = await getDevtoolService().server.getLoggedInUser();
            ...
            const user = await getDevtoolService().server.updateLoggedInUser(update);
