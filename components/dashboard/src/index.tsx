...
import { getURLHash, isDevtoolIo, isWebsiteSlug } from "./utils";

const bootApp = () => {
    // devtool.io specific boot logic
    if (isDevtoolIo()) {
        // Redirect to www website for any website slugs
        if (isWebsiteSlug(window.location.pathname)) {
            window.location.host = "www.devtool.io";
            return;
        }
    }
    ...
    const DevtoolQueryClientProvider = setupQueryClientProvider();
        ...
        <React.StrictMode>
            <ThemeContextProvider>
                <ReloadPageErrorBoundary>
                    <BrowserRouter>
                        <DevtoolQueryClientProvider>
                            {/* This needs to be inside of the DevtoolQueryClientProvider so it can reset queries if needed */}
                            <QueryErrorBoundary>
