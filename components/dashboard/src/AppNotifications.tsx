    ...
    return {
        ...
        message: (
            <span className="text-md">
                ...
                <a className="gp-link" href="https://www.devtool.io/privacy" target="_blank" rel="noreferrer">
                    here
                </a>
                .
            </span>
...
const DEVTOOL_FLEX_INTRODUCTION_COACHMARK_KEY = "devtool_flex_introduction";
const DEVTOOL_FLEX_INTRODUCTION = (updateUser: (user: Partial<UserProtocol>) => Promise<User>) => {
    return {
        id: DEVTOOL_FLEX_INTRODUCTION_COACHMARK_KEY,
        type: "info",
        preventDismiss: true,
        onClose: async () => {
            let dismissSuccess = false;
            try {
                    ...
                    additionalData: {
                        profile: {
                            coachmarksDismissals: {
                                [DEVTOOL_FLEX_INTRODUCTION_COACHMARK_KEY]: new Date().toISOString(),
                            },
                        },
                    },
                });
                ...
                trackEvent("coachmark_dismissed", {
                    name: "devtool-flex-introduction",
                    success: dismissSuccess,
                });
            }
        },
        message: (
            <span className="text-md">
                <b>Introducing Devtool Flex:</b> self-host for free in 3 min or run locally using Devtool Desktop |{" "}
                <a
                    className="text-kumquat-ripe font-bold"
                    href="https://app.devtool.io"
                    target="_blank"
                    rel="noreferrer"
                >
                    Try now
    ...
    useEffect(() => {
        ...
        const updateNotifications = async () => {
            ...
            if (!loading) {
                ...
                if (isDevtoolIo() && currentOrg && billingMode?.mode === "usage-based") {
                ...
                if (isDevtoolIo() && !user?.profile?.coachmarksDismissals[DEVTOOL_FLEX_INTRODUCTION_COACHMARK_KEY]) {
                    notifications.push(DEVTOOL_FLEX_INTRODUCTION((u: Partial<UserProtocol>) => mutateAsync(u)));
                }
            }
...
async function checkForInvalidBillingAddress(org: Organization): Promise<Notification | undefined> {
    try {
        ...
        const subscriptionId = await getDevtoolService().server.findStripeSubscriptionId(attributionId);
        if (!subscriptionId) {
            return undefined;
        }

        const invalidBillingAddress = await getDevtoolService().server.isCustomerBillingAddressInvalid(attributionId);
        if (!invalidBillingAddress) {
            return undefined;
        }

        const stripePortalUrl = await getDevtoolService().server.getStripePortalUrl(attributionId);
