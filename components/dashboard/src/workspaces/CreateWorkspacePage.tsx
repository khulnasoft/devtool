...
export function CreateWorkspacePage() {
    ...
    const createWorkspace = useCallback(
        ...
        async (options?: PartialMessage<CreateAndStartWorkspaceRequest>) => {
            ...
            // if user received an INVALID_DEVTOOL_YML yml for their contextURL they can choose to proceed using default configuration
            if (
                workspaceContext.error &&
                ApplicationError.hasErrorCode(workspaceContext.error) &&
                workspaceContext.error.code === ErrorCodes.INVALID_DEVTOOL_YML
            ) {
    ...
    const continueButtonDisabled = useMemo(() => {
        ...
        if (workspaceContext.error) {
            // For INVALID_DEVTOOL_YML we don't want to disable the button
...
const ErrorMessage: FunctionComponent<ErrorMessageProps> = ({ error, reset, setSelectAccountError }) => {
    ...
    switch (error.code) {
        case ErrorCodes.INVALID_DEVTOOL_YML:
            return (
...
export const RepositoryNotFound: FC<{ error: StartWorkspaceError }> = ({ error }) => {
    ...
    const missingScope =
        authProvider.type === AuthProviderType.GITHUB
            ...
            ? "api"
            : "";
    const authorizeURL = devtoolHostUrl
        .withApi({
            pathname: "/authorize",
    ...
    if (!userScopes.includes(missingScope)) {
        return (
            <RepositoryInputError
                title="The repository may be private. Please authorize Devtool to access private repositories."
    ...
    if (!updatedRecently) {
        return (
            <RepositoryInputError
                title={`Permission to access private repositories has been granted. If you are a member of '${owner}', please try to request access for Devtool.`}
                message={errorMessage}
    ...
    if (authProvider.id.toLocaleLowerCase() === "public-github" && isDevtoolIo()) {
        return (
            <RepositoryInputError
    ...
    return (
        <RepositoryInputError
            title={`Your access token was updated recently. Please try again if the repository exists and Devtool was approved for '${owner}'.`}
            message={errorMessage}
...
export function LimitReachedModal(p: { children: ReactNode }) {
    ...
    return (
        ...
        <Modal visible={true} closeable={false} onClose={() => {}}>
            ...
            <ModalFooter>
                <a href={devtoolHostUrl.asDashboard().toString()}>
                    <Button variant="secondary">Go to Dashboard</Button>
                </a>
                <a href={devtoolHostUrl.with({ pathname: "plans" }).toString()} className="ml-2">
