...
export default function UsageBasedBillingConfig({ hideSubheading = false }: Props) {
    ...
    useEffect(() => {
        if (attributionId) {
            getDevtoolService().server.getPriceInformation(attributionId).then(setPriceInformation);
        }
    }, [attributionId]);
    ...
    const refreshSubscriptionDetails = useCallback(
        async (attributionId: string) => {
            ...
            try {
                getDevtoolService().server.getStripePortalUrl(attributionId).then(setStripePortalUrl);
                getDevtoolService().server.getUsageBalance(attributionId).then(setCurrentUsage);
                const costCenter = await getDevtoolService().server.getCostCenter(attributionId);
    ...
    const subscribeToStripe = useCallback(
        async (stripeParams: { paymentIntentId?: string; redirectStatus: string }) => {
            ...
            try {
                ...
                const newLimit = await getDevtoolService().server.subscribeToStripe(
                    attributionId,
                    paymentIntentId || "",
                    limit,
                );
                ...
                setErrorMessage(
                    ...
                    } Contact support@devtool.io if you believe this is a system error.`,
                );
            }
        },
    ...
    const updateUsageLimit = useCallback(
        async (newLimit: number) => {
            ...
            try {
                await getDevtoolService().server.setUsageLimit(attributionId, newLimit);
                setUsageLimit(newLimit);
