...
import { getDevtoolService, devtoolHostUrl, getIDEFrontendService, IDEFrontendService } from "../service/service";
...
export interface StartWorkspaceProps {
    ...
    /**
     * This flag is used to break the autostart-cycle explained in https://github.com/khulnasoft/devtool/issues/8043
     */
...
export default class StartWorkspace extends React.Component<StartWorkspaceProps, StartWorkspaceState> {
    ...
    componentWillMount() {
        if (this.props.runsInIFrame) {
            this.ideFrontendService = getIDEFrontendService(this.props.workspaceId, sessionId, getDevtoolService());
            this.toDispose.push(
        ...
        try {
            ...
            this.toDispose.push(
                getDevtoolService().registerClient({
    ...
    async startWorkspace(restart = false, forceDefaultConfig = false) {
        ...
        try {
            ...
            if (normalizedError?.code === ErrorCodes.USER_BLOCKED) {
                this.redirectTo(devtoolHostUrl.with({ pathname: "/blocked" }).toString());
                return;
            }
    ...
    /**
     * TODO(gpl) Ideally this can be pushed into the DevtoolService implementation. But to get started we hand-roll it here.
     * @param workspaceId
     * @param options
     * @returns
     */
    ...
    protected async fetchIDEOptions() {
        const ideOptions = await getDevtoolService().server.getIDEOptions();
    ...
    private async onWorkspaceUpdate(workspace?: Workspace) {
        ...
        if (
            ...
            if (contextURL) {
                this.redirectTo(devtoolHostUrl.withContext(contextURL.toString()).toString());
            } else {
    ...
    async ensureWorkspaceAuth(instanceID: string, retry: boolean) {
        ...
        while (attempt <= MAX_ATTEMPTS) {
            ...
            try {
                const authURL = devtoolHostUrl.asWorkspaceAuth(instanceID);
                                ...
                                <ContextMenu
                                    menuEntries={[
                                        ...
                                        },
                                        {
                                            ...
                                            href: devtoolHostUrl.asWorkspacePage().toString(),
                                            target: "_parent",
                                        },
                                    ]}
                                >
                            ...
                            {!useLatest && (
                                <Alert type="info" className="mt-4 w-96">
                                    ...
                                    <a
                                        ...
                                        target="_blank"
                                        rel="noreferrer"
                                        href={devtoolHostUrl.asPreferences().toString()}
                                    >
                                        user preferences
                                    </a>
                                    .
                ...
                statusMessage = (
                    <div>
                        ...
                        <div className="mt-10 flex justify-center">
                            <a target="_parent" href={devtoolHostUrl.asWorkspacePage().toString()}>
                ...
                statusMessage = (
                    <div>
                        ...
                        <div className="mt-10 justify-center flex space-x-2">
                            ...
                            <a target="_parent" href={devtoolHostUrl.asStart(this.state.workspace.id).toString()}>
...
function ImageBuildView(props: ImageBuildViewProps) {
    ...
    useEffect(() => {
        ...
        const toDispose = getDevtoolService().registerClient({
                ...
                <>
                    <div className="mt-6 w-11/12 lg:w-3/5">
                        <p className="text-center text-gray-400 dark:text-gray-500">
                            ...
                            <a
                                href="https://www.devtool.io/docs/configure/workspaces#validate-your-devtool-configuration"
                                target="_blank"
