...
} from "@khulnasoft/devtool-protocol/lib/util/parse-workspace-id";
...
export function WorkspaceSearch(props: Props) {
    ...
    const search = async (page: number = 1) => {
        ...
        if (isDevtoolIo() && !props.user && queryTerm.length === 0) {
            return;
        }
        ...
        try {
            ...
            if (isDevtoolIo() && !query.ownerId && !query.instanceIdOrWorkspaceId && !query.workspaceId) {
                return;
            }

            const result = await getDevtoolService().server.adminGetWorkspaces({
                limit: pageLength,
