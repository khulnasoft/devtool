...
export default function UserDetail(p: { user: User }) {
    ...
    const verifyUser = async () => {
        await updateUser(async (u) => {
            return await getDevtoolService().server.adminVerifyUser(u.id);
        });
    };
    ...
    const toggleBlockUser = async () => {
        await updateUser(async (u) => {
            u.blocked = !u.blocked;
            await getDevtoolService().server.adminBlockUser({
                blocked: u.blocked,
                id: u.id,
            });
            return u;
    ...
    const deleteUser = async () => {
        await updateUser(async (u) => {
            ...
            await getDevtoolService().server.adminDeleteUser(u.id);
            return u;
        });
    };
    ...
    return Object.entries(WorkspaceFeatureFlags)
        ...
        .map((name) => {
            ...
            return {
                ...
                onClick: async () => {
                    await updateUser(async (u) => {
                        return await getDevtoolService().server.adminModifyPermanentWorkspaceFeatureFlag({
                            id: user.id,
                            changes: [
                                {
                                    featureFlag: name,
...
function getRopEntries(user: User, updateUser: UpdateUserFunction): Entry[] {
    const createRopEntry = (name: RoleOrPermission, role?: boolean) => {
        ...
        return {
            ...
            onClick: async () => {
                await updateUser(async (u) => {
                    return await getDevtoolService().server.adminModifyRoleOrPermission({
                        id: user.id,
                        rpp: [
                            {
                                r: name,
