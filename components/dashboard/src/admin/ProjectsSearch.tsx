...
export function ProjectsSearch() {
    ...
    useEffect(() => {
        (async () => {
            if (currentProject) {
                const owner = await getDevtoolService().server.adminGetTeamById(currentProject.teamId);
                if (owner) {
    ...
    const search = async (page: number = 1) => {
        setSearching(true);
        try {
            const result = await getDevtoolService().server.adminGetProjectsBySearchTerm({
                searchTerm,
                limit: pageLength,
