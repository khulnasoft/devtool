...
export default function TeamDetail(props: { team: Team }) {
    ...
    const initialize = () => {
        (async () => {
            const members = await getDevtoolService().server.adminGetTeamMembers(team.id);
            if (members.length > 0) {
                setTeamMembers(members);
            }
        })();
        getDevtoolService().server.adminGetBillingMode(attributionId).then(setBillingMode);
        getDevtoolService().server.adminGetCostCenter(attributionId).then(setCostCenter);
        getDevtoolService().server.adminGetUsageBalance(attributionId).then(setUsageBalance);
    };
    ...
    const setTeamMemberRole = async (userId: string, role: TeamMemberRole) => {
        await getDevtoolService().server.adminSetTeamMemberRole(team!.id, userId, role);
        setTeamMembers(await getDevtoolService().server.adminGetTeamMembers(team!.id));
    };
    return (
        <>
                    ...
                    <Button
                        ...
                        onClick={async () => {
                            if (usageLimit !== undefined) {
                                await getDevtoolService().server.adminSetUsageLimit(attributionId, usageLimit || 0);
                    ...
                    <Button
                        ...
                        onClick={async () => {
                            if (creditNote.credits !== 0 && !!creditNote.note) {
                                await getDevtoolService().server.adminAddUsageCreditNote(
                                    attributionId,
                                    creditNote.credits,
                                    creditNote.note,
                                );
