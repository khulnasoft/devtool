...
export default function WorkspaceDetail(props: { workspace: WorkspaceAndInstance }) {
    ...
    useEffect(() => {
        getDevtoolService().server.adminGetUser(props.workspace.ownerId).then(setUser);
        getDevtoolService().server.adminGetWorkspaceInstances(props.workspace.workspaceId).then(setWorkspaceInstances);
    }, [props.workspace, workspace.workspaceId]);

    const stopWorkspace = async () => {
        try {
            setActivity(true);
            await getDevtoolService().server.adminForceStopWorkspace(workspace.workspaceId);
    ...
    const reload = async () => {
        try {
            ...
            const [ws, workspaceInstances] = await Promise.all([
                await getDevtoolService().server.adminGetWorkspace(workspace.workspaceId),
                await getDevtoolService().server.adminGetWorkspaceInstances(workspace.workspaceId),
            ]);
            setWorkspace(ws);
        ...
        <div className="app-container">
            <div className="flex mt-8">
                ...
                <Button
                    ...
                    className="ml-3"
                    onClick={() => {
                        window.location.href = new DevtoolHostUrl(window.location.href)
                            .with({
                                    ...
                                    ? [
                                          {
                                              label: "Restore & Pin",
                                              onClick: async () => {
                                                  await getDevtoolService().server.adminRestoreSoftDeletedWorkspace(
