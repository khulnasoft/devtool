...
function createDevtoolService<C extends DevtoolClient, S extends DevtoolServer>() {
    let host = devtoolHostUrl.asWebsocket().with({ pathname: DevtoolServerPath }).withApi();
    ...
    return new DevtoolServiceImpl<C, S>(proxy, { onReconnect });
}
        ...
        return originalCreateWebSocket.call(
            ...
            new Proxy(WebSocket, {
                construct(target: any, argArray) {
                    ...
                    instrumentWebSocket(webSocket, "devtool");
                    return webSocket;
                },
            }),
        );
...
export function getDevtoolService(): DevtoolService {
    const w = window as any;
    const _gp = w._gp || (w._gp = {});
    let service = _gp.devtoolService;
    if (!service) {
        service = _gp.devtoolService = createDevtoolService();
...
export function getIDEFrontendService(workspaceID: string, sessionId: string, service: DevtoolService) {
...
export class IDEFrontendService implements IDEFrontendDashboardService.IServer {
    ...
    constructor(
        ...
        private service: DevtoolService,
        ...
        window.addEventListener("unload", () => {
            ...
            const devtoolHostUrl = new DevtoolHostUrl(window.location.toString());
            const url = devtoolHostUrl.withApi({ pathname: `/auth/workspacePageClose/${this.instanceID}` }).toString();
    ...
    private async redirectToCustomUrl(info: IDEFrontendDashboardService.Info) {
        const isDataOps = await getExperimentsClient().getValueAsync("dataops", false, {
            ...
            devtoolHost: devtoolHostUrl.toString(),
        });
    ...
    private async auth() {
        ...
        const url = devtoolHostUrl.asWorkspaceAuth(this.instanceID).toString();
        await fetch(url, {
            credentials: "include",
        });
    }
    ...
    openDesktopIDE(url: string): void {
        ...
        try {
            ...
            if (
                ...
                !desktopLink.href.startsWith("jetbrains://gateway/io.devtool.toolbox.gateway/")
            ) {
                redirect = false;
            }
        } catch (e) {
