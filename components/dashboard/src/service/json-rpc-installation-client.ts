...
export class JsonRpcInstallationClient implements PromiseClient<typeof InstallationService> {
    async getInstallationWorkspaceDefaultImage(
        ...
        const result = await getDevtoolService().server.getDefaultWorkspaceImage({});
    ...
    async listBlockedRepositories(
        ...
        const info = await getDevtoolService().server.adminGetBlockedRepositories({
            limit: 100,
            offset: 0,
            orderBy: "urlRegexp",
            orderDir: "asc",
    ...
    async createBlockedRepository(
        ...
        const info = await getDevtoolService().server.adminCreateBlockedRepository(
            request.urlRegexp,
            request.blockUser,
    ...
    async deleteBlockedRepository(
        ...
        await getDevtoolService().server.adminDeleteBlockedRepository(request.blockedRepositoryId);
    ...
    async listBlockedEmailDomains(
        ...
        const info = await getDevtoolService().server.adminGetBlockedEmailDomains();
    ...
    async createBlockedEmailDomain(
        ...
        await getDevtoolService().server.adminSaveBlockedEmailDomain({
    ...
    async getOnboardingState(
        ...
        const info = await getDevtoolService().server.getOnboardingState();
    ...
    async getInstallationConfiguration(
        ...
        const config = await getDevtoolService().server.getConfiguration();
