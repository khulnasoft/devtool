...
export class JsonRpcWorkspaceClient implements PromiseClient<typeof WorkspaceService> {
    ...
    async getWorkspace(request: PartialMessage<GetWorkspaceRequest>): Promise<GetWorkspaceResponse> {
        ...
        const info = await getDevtoolService().server.getWorkspace(request.workspaceId);
    ...
    async listWorkspaces(
        ...
        let resultTotal = 0;
        const results = await getDevtoolService().server.getWorkspaces({
            limit,
    ...
    async createAndStartWorkspace(
        ...
        const response = await getDevtoolService().server.createWorkspace({
    ...
    async startWorkspace(request: PartialMessage<StartWorkspaceRequest>, _options?: CallOptions | undefined) {
        ...
        await getDevtoolService().server.startWorkspace(request.workspaceId, {
    ...
    async getWorkspaceDefaultImage(
        ...
        const response = await getDevtoolService().server.getDefaultWorkspaceImage({
    ...
    async sendHeartBeat(
        ...
        await getDevtoolService().server.sendHeartBeat({
    ...
    async getWorkspaceOwnerToken(
        ...
        const ownerToken = await getDevtoolService().server.getOwnerToken(request.workspaceId);
    ...
    async getWorkspaceEditorCredentials(
        ...
        const credentials = await getDevtoolService().server.getIDECredentials(request.workspaceId);
    ...
    async updateWorkspace(
        ...
        const server = getDevtoolService().server;
    ...
    async stopWorkspace(
        ...
        await getDevtoolService().server.stopWorkspace(request.workspaceId);
    ...
    async deleteWorkspace(
        ...
        await getDevtoolService().server.deleteWorkspace(request.workspaceId);
    ...
    async parseContextURL(
        ...
        const context = await getDevtoolService().server.resolveContext(request.contextUrl);
    ...
    async listWorkspaceClasses(
        ...
        const list = await getDevtoolService().server.getSupportedWorkspaceClasses();
    ...
    async createWorkspaceSnapshot(
        ...
        const snapshotId = await getDevtoolService().server.takeSnapshot({
    ...
    async waitForWorkspaceSnapshot(
        ...
        await getDevtoolService().server.waitForSnapshot(req.snapshotId);
    ...
    async updateWorkspacePort(
        ...
        getDevtoolService().server.openPort(req.workspaceId, {
