...
type Client struct {
	...
	// Config values to be set on clone provided through `.devtool.yml`
	...
	// if true will run git command as devtool user (should be executed as root that has access to sudo in this case)
	RunAsDevtoolUser bool
...
func (c *Client) GitWithOutput(ctx context.Context, ignoreErr *string, subcommand string, args ...string) (out []byte, err error) {
	...
	env = append(env, "HOME=/home/devtool")
	...
	cmdName := "git"
	if c.RunAsDevtoolUser {
		cmdName = "sudo"
		fullArgs = append([]string{"-u", "devtool", "git"}, fullArgs...)
	}
