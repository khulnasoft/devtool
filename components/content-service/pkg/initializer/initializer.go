...
import (
	...
	"github.com/khulnasoft/devtool/content-service/pkg/archive"
...
const (
	...
	WorkspaceReadyFile = ".devtool/ready"

	// DevtoolUID is the user ID of the devtool user
	DevtoolUID = 33333

	// DevtoolGID is the group ID of the devtool user group
	DevtoolGID = 33333
...
type NewFromRequestOpts struct {
	// ForceDevtoolUserForGit forces devtool:devtool ownership on all files produced by the Git initializer.
	// For FWB workspaces the content init is run from supervisor which runs as UID 0. Using this flag, the
	// Git content is forced to the Devtool user. All other content (backup, prebuild, snapshot) will already
	// have the correct user.
	ForceDevtoolUserForGit bool
}
...
func NewFromRequest(ctx context.Context, loc string, rs storage.DirectDownloader, req *csapi.WorkspaceInitializer, opts NewFromRequestOpts) (i Initializer, err error) {
	...
	if _, ok := spec.(*csapi.WorkspaceInitializer_Empty); ok {
		...
		initializer, err = newGitInitializer(ctx, loc, ir.Git, opts.ForceDevtoolUserForGit)
		...
		for _, gi := range ir.Prebuild.Git {
			gitinit, err := newGitInitializer(ctx, loc, gi, opts.ForceDevtoolUserForGit)
			if err != nil {
				return nil, err
			}
...
// Returns gRPC errors.
func newGitInitializer(ctx context.Context, loc string, req *csapi.GitInitializer, forceDevtoolUser bool) (*GitInitializer, error) {
	if req.Config == nil {
	...
	return &GitInitializer{
		Client: git.Client{
			...
			RunAsDevtoolUser:   forceDevtoolUser,
...
// Some workspace content may have a `/dst/.devtool` file or directory. That would break
// the workspace ready file placement (see https://github.com/khulnasoft/devtool/issues/7694).
// This function ensures that workspaces do not have a `.devtool` file or directory present.
func EnsureCleanDotDevtoolDirectory(ctx context.Context, wspath string) error {
	...
	dotDevtool := filepath.Join(wspath, ".devtool")
	stat, err := os.Stat(dotDevtool)
	...
	candidateFN := filepath.Join(wspath, ".devtool.yaml")
	if _, err := os.Stat(candidateFN); err == nil {
		// Our candidate file already exists, hence we cannot just move things.
		// As fallback we'll delete the .devtool entry.
		return os.RemoveAll(dotDevtool)
	}

	err = mv(dotDevtool, candidateFN)
	if err != nil {
		return err
	}
...
func PlaceWorkspaceReadyFile(ctx context.Context, wspath string, initsrc csapi.WorkspaceInitSource, metrics csapi.InitializerMetrics, uid, gid int) (err error) {
	...
	devtoolDir := filepath.Join(wspath, filepath.Dir(WorkspaceReadyFile))
	err = os.MkdirAll(devtoolDir, 0777)
	if err != nil {
		return xerrors.Errorf("cannot create directory for workspace ready file: %w", err)
	}
	err = os.Chown(devtoolDir, uid, gid)
	if err != nil {
