...
func main() {
	...
	url, err := url.Parse(wsInfo.DevtoolHost)
	if err != nil {
		log.WithError(err).Errorf("failed to parse DevtoolHost %s", wsInfo.DevtoolHost)
		return
	}
	domain := url.Hostname()
	...
	args = append(args, "--install-builtin-extension", "devtool.devtool-theme")
	...
	cmdEnv := append(os.Environ(), fmt.Sprintf("DEVTOOL_CODE_HOST=%s", domain))
	log.WithField("cost", time.Now().Local().Sub(startTime).Milliseconds()).Info("starting server")
	if err := syscall.Exec(Code, append([]string{"devtool-code"}, args...), cmdEnv); err != nil {
...
func getExtensions(repoRoot string) (extensions []Extension, err error) {
	...
	if err != nil {
		...
		err = errors.New("read .devtool.yml file failed: " + err.Error())
		return
	}
	var config *devtool.DevtoolConfig
	if err = yaml.Unmarshal(data, &config); err != nil {
		err = errors.New("unmarshal .devtool.yml file failed" + err.Error())
		return
	}
