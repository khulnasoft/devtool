...
import io.devtool.devtoolprotocol.api.entities.WorkspaceInfo
...
class GatewayDevtoolClient(
    private val lifetimeDefinition: LifetimeDefinition,
    private val devtoolHost: String
) : DevtoolClient() {
    ...
    private fun scheduleTimeout(reason: String) {
        ...
        timeoutJob = GlobalScope.launch {
            thisLogger().info("$devtoolHost: connection times out in $timeoutDelayInMinutes minutes: $reason")
    ...
    private fun cancelTimeout(reason: String) {
        if (timeoutJob?.isActive == true) {
            thisLogger().info("$devtoolHost: canceled connection timeout: $reason")
            timeoutJob!!.cancel()
        }
    }
        ...
        syncJob = GlobalScope.launch {
            for (id in listeners.keys) {
                ...
                try {
                    ...
                    thisLogger().error("$devtoolHost: $id: failed to sync", t)
                }
            }
        }
    }
