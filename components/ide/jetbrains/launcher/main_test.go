...
func TestGetProductConfig(t *testing.T) {
	...
	actual := getProductConfig(&protocol.DevtoolConfig{
...
func TestParseDevtoolConfig(t *testing.T) {
	devtoolConfig, _ := parseDevtoolConfig("testdata")
	assert.Equal(t, 1, len(devtoolConfig.Jetbrains.Intellij.Plugins))
	assert.Equal(t, "both", devtoolConfig.Jetbrains.Intellij.Prebuilds.Version)
	assert.Equal(t, "-Xmx3g", devtoolConfig.Jetbrains.Intellij.Vmoptions)
	assert.Equal(t, "-Xmx4096m -XX:MaxRAMPercentage=75", devtoolConfig.Jetbrains.Goland.Vmoptions)
}
...
func TestUpdateVMOptions(t *testing.T) {
	tests := []struct {
		...
		{"idea64.vmoptions (DEVTOOL_CPU_COUNT)", "intellij", map[string]string{"INTELLIJ_VMOPTIONS": "-Xmx4096m", "DEVTOOL_CPU_COUNT": "1"}, "-Xms128m\n-Xmx2g\n-Dsun.tools.attach.tmp.only=true", "-Xms128m\n-Xmx4096m\n-Dsun.tools.attach.tmp.only=true\n-XX:+UseContainerSupport\n-XX:ActiveProcessorCount=1\n-Dfreeze.reporter.profiling=false\n-Dgtw.disable.exit.dialog=true\n-Djdk.configure.existing=true"},
		{"idea64.vmoptions (DEVTOOL_CPU_COUNT 2)", "intellij", map[string]string{"INTELLIJ_VMOPTIONS": "-Xmx4096m", "DEVTOOL_CPU_COUNT": "12"}, "-Xms128m\n-Xmx2g\n-Dsun.tools.attach.tmp.only=true", "-Xms128m\n-Xmx4096m\n-Dsun.tools.attach.tmp.only=true\n-XX:+UseContainerSupport\n-XX:ActiveProcessorCount=12\n-Dfreeze.reporter.profiling=false\n-Dgtw.disable.exit.dialog=true\n-Djdk.configure.existing=true"},
