...
import (
	...
	"github.com/khulnasoft/devtool/common-go/util"
	devtool "github.com/khulnasoft/devtool/devtool-protocol"
	"github.com/khulnasoft/devtool/jetbrains/launcher/pkg/constant"
...
type LaunchContext struct {
	...
	// shouldWaitBackendPlugin is controlled by env DEVTOOL_WAIT_IDE_BACKEND
...
func main() {
	...
	// supervisor refer see https://github.com/khulnasoft/devtool/blob/main/components/supervisor/pkg/supervisor/supervisor.go#L961
	shouldWaitBackendPlugin := os.Getenv("DEVTOOL_WAIT_IDE_BACKEND") == "true"
	debugEnabled := os.Getenv("SUPERVISOR_DEBUG_ENABLE") == "true"
	preferToolbox := os.Getenv("DEVTOOL_PREFER_TOOLBOX") == "true"
...
// isBackendPluginReady checks if the backend plugin is ready via backend plugin CLI DevtoolCLIService.kt
func isBackendPluginReady(ctx context.Context, backendPort string, shouldWaitBackendPlugin bool) error {
	...
	url, err := url.Parse("http://localhost:" + backendPort + "/api/devtool/cli?op=metrics")
	if err != nil {
		return err
	}
...
func resolveToolboxLink(wsInfo *supervisor.WorkspaceInfoResponse) (string, error) {
	devtoolUrl, err := url.Parse(wsInfo.DevtoolHost)
	if err != nil {
		return "", err
	}
	...
	link := url.URL{
		Scheme:   "jetbrains",
		Host:     "gateway",
		Path:     "io.devtool.toolbox.gateway/open-in-toolbox",
		RawQuery: fmt.Sprintf("host=%s&workspaceId=%s&debugWorkspace=%t", devtoolUrl.Hostname(), wsInfo.WorkspaceId, debugWorkspace),
	}
...
func resolveGatewayLink(backendPort string, wsInfo *supervisor.WorkspaceInfoResponse) (string, error) {
	...
	link := url.URL{
		...
		Host:     "connect",
		Fragment: fmt.Sprintf("devtoolHost=%s&workspaceId=%s&backendPort=%s&debugWorkspace=%t", devtoolUrl.Hostname(), wsInfo.WorkspaceId, backendPort, debugWorkspace),
	}
	return link.String(), nil
}
...
func resolveJsonLink(backendPort string) (string, error) {
	var (
		hostStatusUrl = "http://localhost:" + backendPort + "/codeWithMe/unattendedHostStatus?token=devtool"
...
func terminateIDE(backendPort string) error {
	var (
		hostStatusUrl = "http://localhost:" + backendPort + "/codeWithMe/unattendedHostStatus?token=devtool&exit=true"
...
func launch(launchCtx *LaunchContext) {
	...
	devtoolConfig, err := parseDevtoolConfig(projectDir)
	if err != nil {
		log.WithError(err).Error("failed to parse .devtool.yml")
	}
	...
	err = configureVMOptions(devtoolConfig, launchCtx.alias, launchCtx.vmOptionsFile)
	if err != nil {
	...
	err = installPlugins(devtoolConfig, launchCtx)
	...
	if err != nil {
		log.WithError(err).Error("failed to install devtool-remote plugin")
	}
...
func run(launchCtx *LaunchContext) {
	...
	cmd.Env = append(cmd.Env, "JETBRAINS_DEVTOOL_BACKEND_KIND="+launchCtx.alias)
	workspaceUrl, err := url.Parse(launchCtx.wsInfo.WorkspaceUrl)
	if err == nil {
		cmd.Env = append(cmd.Env, "JETBRAINS_DEVTOOL_WORKSPACE_HOST="+workspaceUrl.Hostname())
	}
	// Enable host status endpoint
	cmd.Env = append(cmd.Env, "CWM_HOST_STATUS_OVER_HTTP_TOKEN=devtool")
...
func configureVMOptions(config *devtool.DevtoolConfig, alias string, vmOptionsPath string) error {
...
func updateVMOptions(
	...
	// Devtool's default customization
	var devtoolVMOptions []string
	devtoolVMOptions = append(devtoolVMOptions, "-Dgtw.disable.exit.dialog=true")
	// temporary disable auto-attach of the async-profiler to prevent JVM crash
	// see https://youtrack.jetbrains.com/issue/IDEA-326201/SIGSEGV-on-startup-2023.2-IDE-backend-on-devtool.io?s=SIGSEGV-on-startup-2023.2-IDE-backend-on-devtool.io
	devtoolVMOptions = append(devtoolVMOptions, "-Dfreeze.reporter.profiling=false")
	if alias == "intellij" {
		devtoolVMOptions = append(devtoolVMOptions, "-Djdk.configure.existing=true")
	}
	// container relevant options
	devtoolVMOptions = append(devtoolVMOptions, "-XX:+UseContainerSupport")
	cpuCount := os.Getenv("DEVTOOL_CPU_COUNT")
	...
	if err == nil && parsedCPUCount > 0 && parsedCPUCount <= 16 {
		devtoolVMOptions = append(devtoolVMOptions, "-XX:ActiveProcessorCount="+cpuCount)
	}
	vmoptions := deduplicateVMOption(ideaVMOptionsLines, devtoolVMOptions, filterFunc)
	...
	// project-defined vmoptions (.devtool.yml)
	if config != nil {
...
func collectSyncSources(launchCtx *LaunchContext, target SyncTarget) ([]string, error) {
	...
	for _, srcDir := range []string{
		fmt.Sprintf("%s/.devtool/jetbrains/%s", userHomeDir, target),
		fmt.Sprintf("%s/.devtool/jetbrains/%s/%s", userHomeDir, launchCtx.alias, target),
		fmt.Sprintf("%s/.devtool/jetbrains/%s", launchCtx.projectDir, target),
		fmt.Sprintf("%s/.devtool/jetbrains/%s/%s", launchCtx.projectDir, launchCtx.alias, target),
	} {
...
func installPlugins(config *devtool.DevtoolConfig, launchCtx *LaunchContext) error {
...
func getPlugins(config *devtool.DevtoolConfig, alias string) ([]string, error) {
	var plugins []string
...
func getProductConfig(config *devtool.DevtoolConfig, alias string) *devtool.JetbrainsProduct {
	defer func() {
	...
	productConfig, ok := v.(*devtool.JetbrainsProduct)
	if !ok {
		return nil
	}
...
func linkRemotePlugin(launchCtx *LaunchContext) error {
	...
	remotePluginDir := remotePluginsFolder + "/devtool-remote"
	if err := os.MkdirAll(remotePluginsFolder, 0755); err != nil {
		return err
	}

	// added for backwards compatibility, can be removed in the future
	sourceDir := "/ide-desktop-plugins/devtool-remote-" + os.Getenv("JETBRAINS_BACKEND_QUALIFIER")
	_, err := os.Stat(sourceDir)
	if err == nil {
		return safeLink(sourceDir, remotePluginDir)
	}

	return safeLink("/ide-desktop-plugins/devtool-remote", remotePluginDir)
}
...
func configureToolboxCliProperties(backendDir string) error {
	...
	debuggingToolbox := os.Getenv("DEVTOOL_TOOLBOX_DEBUGGING")
