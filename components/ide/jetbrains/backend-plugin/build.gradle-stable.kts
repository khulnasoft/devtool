...
var pluginVersion = "${properties("pluginVersion")}-${properties("devtoolVersion")}"
...
project(":") {
    kotlin {
        ...
        sourceSets["main"].kotlin.exclude("io/devtool/jetbrains/remote/${excludedPackage}/**")

        if (properties("platformType") == "RD") {
            print("Rider: exclude unnecessary files")
            sourceSets["main"].kotlin.exclude("**/DevtoolForceUpdateMavenProjectsActivity.kt")
...
dependencies {
    ...
    implementation(project(":devtool-protocol")) {
        artifact {
            type = "jar"
        }
    }
...
tasks.withType<KotlinCompile> {
    if (properties("platformType") == "RD") {
        print("Rider: exclude unnecessary files")
        exclude("**/DevtoolForceUpdateMavenProjectsActivity.kt")
        exclude("**/maven.xml")
    }
}
