...
DEBUG_PORT=44444
JB_QUALIFIER="latest"
TEST_REPO=https://github.com/devtool-samples/spring-petclinic
RUN_FROM="release"
...
TEST_PLUGIN_DIR="$TEST_PLUGINS_DIR/devtool-remote"
rm -rf $TEST_PLUGIN_DIR

DEVTOOL_PLUGIN_DIR=/workspace/devtool/components/ide/jetbrains/backend-plugin
$DEVTOOL_PLUGIN_DIR/gradlew -PenvironmentName="$JB_QUALIFIER" buildPlugin

# TODO(ak) actually should be gradle task to make use of output
DEVTOOL_PLUGIN_DIST="$DEVTOOL_PLUGIN_DIR/build/distributions/devtool-remote.zip"
unzip $DEVTOOL_PLUGIN_DIST -d $TEST_PLUGINS_DIR
...
export CWM_HOST_STATUS_OVER_HTTP_TOKEN=devtool
...
export JETBRAINS_DEVTOOL_BACKEND_KIND=intellij
