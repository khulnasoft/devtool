...
import io.devtool.devtoolprotocol.api.entities.WorkspaceInstancePort
import io.devtool.supervisor.api.Info
import io.devtool.supervisor.api.Status
import io.devtool.supervisor.api.Status.PortVisibility
...
class DevtoolClientProjectSessionTracker(private val project: Project) : Disposable {
    ...
    private fun getForwardedPortUrl(port: PortsStatus): String {
        val localHostUri = serviceOrNull<DevtoolPortForwardingService>()
    ...
    private suspend fun makePortPublic(workspaceId: String, port: PortsStatus) {
        ...
        p.visibility = io.devtool.devtoolprotocol.api.entities.PortVisibility.PUBLIC.toString()
        p.url = port.exposed.url

        try {
            manager.client.server.openPort(workspaceId, p).await()
        } catch (e: Exception) {
            thisLogger().error("devtool: failed to open port ${port.localPort}: ", e)
        }
    }
    ...
    private val portsObserveJob = GlobalScope.launch {
        ...
        val status = StatusServiceGrpc.newStub(DevtoolManager.supervisorChannel)
        while (isActive) {
            try {
                ...
                thisLogger().error("devtool: failed to stream ports status: ", t)
            }
            delay(1000L)
        }
    }
