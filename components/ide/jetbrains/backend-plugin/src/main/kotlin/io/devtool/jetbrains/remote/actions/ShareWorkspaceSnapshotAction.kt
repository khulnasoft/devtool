...
import io.devtool.devtoolprotocol.api.entities.Error
import io.devtool.devtoolprotocol.api.entities.TakeSnapshotOptions
...
class ShareWorkspaceSnapshotAction : AnAction() {
    ...
    override fun actionPerformed(event: AnActionEvent) {
        manager.pendingInfo.thenAccept { workspaceInfo ->
            manager.trackEvent(
                    "jb_execute_command_devtool_workspace", mapOf(
                    "action" to "snapshot"
                )
            )
            ...
            manager.client.server.takeSnapshot(takeSnapshotOptions).whenComplete { snapshotId, t ->
                if (t != null) {
                    ...
                    thisLogger().error("devtool: failed to capture workspace snapshot", t)
                } else {
                    thisLogger().warn("devtool: snapshot started ($snapshotId)")
                    val notification = manager.notificationGroup.createNotification(
                    ...
                    manager.client.server.waitForSnapshot(snapshotId).whenComplete { _, t ->
                        ...
                        val copyUrlAction = NotificationAction.createSimple("Copy URL to Clipboard") {
                            val uri = "${workspaceInfo.devtoolHost}#snapshot/$snapshotId";
