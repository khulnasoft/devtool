...
import io.devtool.jetbrains.remote.utils.LocalHostUri
...
class DevtoolCLIService : RestService() {
    ...
    override fun execute(urlDecoder: QueryStringDecoder, request: FullHttpRequest, context: ChannelHandlerContext): String? {
        ...
        /**
         * prod: curl http://localhost:63342/api/devtool/cli?op=metrics
         * dev:  curl http://localhost:63343/api/devtool/cli?op=metrics
         *
        ...
        if (operation == "preview") {
            ...
            return withClient(request, context) { project ->
                ...
                val devtoolPortForwardingService = serviceOrNull<DevtoolPortForwardingService>()

                if (localHostUriMetadata.isPresent && devtoolPortForwardingService != null) {
                    var localHostUriFromPort = Optional.empty<URI>()

                    application.invokeAndWait {
                        localHostUriFromPort = devtoolPortForwardingService
    ...
    private fun parseFilePath(path: String): Path? {
        return try {
            ...
            thisLogger().warn("devtool cli: failed to parse file path:", e)
            null
        }
    }

    companion object {
        const val SERVICE_NAME = "devtool/cli"
    }
}
