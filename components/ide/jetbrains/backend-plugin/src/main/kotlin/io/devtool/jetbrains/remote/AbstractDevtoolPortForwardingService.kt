...
import io.devtool.supervisor.api.Status.PortsStatus
...
abstract class AbstractDevtoolPortForwardingService : DevtoolPortForwardingService {
    companion object {
        const val FORWARDED_PORT_LABEL = "ForwardedByDevtool"
        const val EXPOSED_PORT_LABEL = "ExposedByDevtool"
    }

    private val perClientPortForwardingManager = service<PerClientPortForwardingManager>()
    private val ignoredPortsForNotificationService = service<DevtoolIgnoredPortsForNotificationService>()
    ...
    private fun start() {
        ...
        if (isLocalPortForwardingDisabled()) {
            thisLogger().warn("devtool: Local port forwarding is disabled.")
        }
            ...
            try {
                ...
                when (throwable) {
                    is InterruptedException, is CancellationException -> {
                        cancel("devtool: Stopped observing ports list due to an expected interruption.")
                    }

                    else -> {
                        thisLogger().warn(
                            "devtool: Got an error while trying to get ports list from Supervisor. " +
    ...
    private fun observePortsList(): CompletableFuture<Void> {
        ...
        val statusServiceStub = StatusServiceGrpc.newStub(DevtoolManager.supervisorChannel)
        ...
        val portsStatusResponseObserver = object :
            ...
            override fun beforeStart(request: ClientCallStreamObserver<Status.PortsStatusRequest>) {
                lifetime.onTerminationOrNow { request.cancel("devtool: Service lifetime terminated.", null) }
            }
    ...
    private fun isLocalPortForwardingDisabled(): Boolean {
        return System.getenv("DEVTOOL_DISABLE_JETBRAINS_LOCAL_PORT_FORWARDING")?.toBoolean() ?: false
    }
    ...
    private fun startForwarding(portStatus: PortsStatus) {
        ...
        try {
            ...
            if (throwable !is PortAlreadyForwardedException) {
                thisLogger().warn("devtool: Caught an exception while forwarding port: ${throwable.message}")
            }
        }
    }
