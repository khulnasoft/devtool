...
import io.devtool.devtoolprotocol.api.entities.RemoteTrackMessage
import io.devtool.jetbrains.remote.services.HeartbeatService
import io.devtool.jetbrains.remote.utils.Retrier.retry
import io.devtool.supervisor.api.*
import io.devtool.supervisor.api.Info.WorkspaceInfoResponse
import io.devtool.supervisor.api.Notification.*
...
@Service
class DevtoolManager : Disposable {
    ...
    private val backendKind = System.getenv("JETBRAINS_DEVTOOL_BACKEND_KIND") ?: "unknown"
    ...
    init {
        ...
        // rate(devtool_jb_backend_low_memory_after_gc_total[5m])
        val lowMemoryCounter = Counter.build()
            .name("devtool_jb_backend_low_memory_after_gc")
    ...
    init {
        val monitoringJob = GlobalScope.launch {
            ...
            // max_over_time(devtool_jb_backend_memory_used_bytes[5m:])/max_over_time(devtool_jb_backend_memory_max_bytes[5m:])
            val allocatedGauge = Gauge.build()
                .name("devtool_jb_backend_memory_max_bytes")
                .help("Total allocated memory of JB backend in bytes.")
                .labelNames("product", "qualifier")
                .register(registry)
            val usedGauge = Gauge.build()
                .name("devtool_jb_backend_memory_used_bytes")
            ...
            while (isActive) {
                ...
                try {
                    ...
                    thisLogger().error("devtool: failed to push monitoring metrics:", t)
                }
                delay(5000)
            }
        }
        ...
        GlobalScope.launch {
            ...
            try {
                ...
                if (response.statusCode() == 200) {
                    ...
                    // the output format should be constant (includes `Devtool gateway link`)
                    // which will be checked in integration test gateway_test.go
                    thisLogger().warn(
                        "\n\n\n*********************************************************\n\n" +
                                "Devtool gateway link: $gatewayLink" +
                                "\n\n*********************************************************\n\n\n"
                    )
                } else {
                ...
                thisLogger().error("devtool: failed to resolve gateway link:", t)
            }
        }
    }
    ...
    val notificationGroup = NotificationGroupManager.getInstance().getNotificationGroup("Devtool Notifications")
    private val notificationsJob = GlobalScope.launch {
        ...
        while (isActive) {
            try {
                ...
                thisLogger().error("devtool: failed to stream notifications: ", t)
            }
            delay(1000L)
        }
    }
    ...
    private val serverJob = GlobalScope.launch {
        ...
        val plugin = PluginManagerCore.getPlugin(PluginId.getId("io.devtool.jetbrains.remote"))!!
        val connect = {
            val originalClassLoader = Thread.currentThread().contextClassLoader
            try {
                val proxies = CommonProxy.getInstance().select(URL(info.devtoolHost))
        ...
        val devtoolHost = info.devtoolApi.host
        ...
        try {
            while (kotlin.coroutines.coroutineContext.isActive) {
                try {
                    ...
                    thisLogger().warn("$devtoolHost: connection closed, reconnecting after $reconnectionDelay milliseconds: $closeReason")
                } catch (t: Throwable) {
        ...
        thisLogger().warn("$devtoolHost: connection permanently closed: $closeReason")
    }
    init {
        lifetime.onTerminationOrNow {
            serverJob.cancel()
    ...
    private val metricsJob = GlobalScope.launch {
        ...
        while (isActive) {
            try {
                ...
                thisLogger().error("devtool: failed to retrieve resource status: ", t)
            }
            delay(1000L)
        }
    }
    ...
    fun openUrlFromAction(url: String) {
        trackEvent("jb_execute_command_devtool_open_link", mapOf("url" to url))
