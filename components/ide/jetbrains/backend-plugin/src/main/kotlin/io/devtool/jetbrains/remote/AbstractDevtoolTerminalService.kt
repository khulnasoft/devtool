...
import io.devtool.supervisor.api.Status
import io.devtool.supervisor.api.StatusServiceGrpc
import io.devtool.supervisor.api.TerminalOuterClass
import io.devtool.supervisor.api.TerminalServiceGrpc
...
abstract class AbstractDevtoolTerminalService(project: Project) : Disposable {
    private val lifetime = defineNestedLifetime()
    private val terminalServiceFutureStub = TerminalServiceGrpc.newFutureStub(DevtoolManager.supervisorChannel)
    private val terminalServiceStub = TerminalServiceGrpc.newStub(DevtoolManager.supervisorChannel)
    private val statusServiceStub = StatusServiceGrpc.newStub(DevtoolManager.supervisorChannel)
    ...
    protected fun start() {
        ...
        runJob(lifetime) {
            try {
                ...
                thisLogger().info("devtool: attaching tasks ${tasks.size}, terminals ${terminals.size}")
                if (tasks.isEmpty() && terminals.isEmpty()) {
                    return@runJob
                }
                // see internal chat https://devtool.slack.com/archives/C02BRJLGPGF/p1716540080028119
                delay(5000L)
                application.invokeLater {
                    createTerminalsAttachedToTasks(terminals, tasks)
                }
            } catch (e: TimeoutCancellationException) {
                thisLogger().error("devtool: timeout while fetching tasks or terminals", e)
            } catch (e: Exception) {
                thisLogger().error("devtool: error while attaching tasks", e)
            }
        }
    }
    ...
    private fun createTerminalsAttachedToTasks(
        ...
        tasks.forEachIndexed { index, task ->
            ...
            if (terminal == null) {
                thisLogger().warn("devtool: found no terminal for task ${task.id}, expecting ${task.terminal}")
                return@forEachIndexed
            }
            val title = terminal.title.takeIf { !it.isNullOrBlank() } ?: "Devtool Task ${index + 1}"
            thisLogger().info("devtool: attaching task ${terminal.title} (${task.terminal}) with title $title")
            createAttachedSharedTerminal(title, terminal)
            thisLogger().info("devtool: attached task ${terminal.title} (${task.terminal})")
        }
    }
    ...
    private tailrec suspend fun getSupervisorTasksList(): List<Status.TaskStatus> {
        ...
        try {
            ...
            thisLogger().error(
                "devtool: Got an error while trying to get tasks list from Supervisor. Trying again in one second.",
                throwable
            )
        }
    ...
    private tailrec suspend fun getSupervisorTerminalsList(): List<TerminalOuterClass.Terminal> {
        ...
        try {
            ...
            thisLogger().error(
                "devtool: Got an error while trying to get terminals list from Supervisor. Trying again in one second.",
                throwable
            )
        }
    ...
    private fun listenForTaskTerminationAndTitleChanges(
        ...
        while (hasOpenSessions) {
                    ...
                    override fun beforeStart(request: ClientCallStreamObserver<TerminalOuterClass.ListenTerminalRequest>) {
                        ...
                        shellTerminalWidget.addListener(object : TerminalWidgetListener {
                            override fun allSessionsClosed(widget: TerminalWidget) {
                                ...
                                request.cancel("devtool: Terminal closed on the client.", null)
                            }
                        })
                    }
            ...
            try {
                ...
                thisLogger().error(
                    "devtool: got an error while listening to '${supervisorTerminal.title}' terminal. Trying again in one second.",
                    throwable
                )
            }
        ...
        shellTerminalWidget.addListener(object : TerminalWidgetListener {
            override fun allSessionsClosed(widget: TerminalWidget) {
                runJob(lifetime) {
                    delay(5000)
                    try {
                        thisLogger().info("devtool: shutdown task ${supervisorTerminal.title} (${supervisorTerminal.alias})")
                        ...
                        thisLogger().error(
                            "devtool: got an error while shutting down '${supervisorTerminal.title}' terminal.",
                            throwable
                        )
                    }
                }
    ...
    private fun closeTerminalWidgetWhenClientGetsClosed(
        ...
        lifetime.onTerminationOrNow {
            thisLogger().debug("devtool: closing task terminal service ${supervisorTerminal.title} (${supervisorTerminal.alias})")
            shellTerminalWidget.close()
        }
    }
}
